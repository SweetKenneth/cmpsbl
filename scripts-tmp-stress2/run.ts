...
interface FileResult {
  file: string; expectedLang: string; detectedLang: string; langConfidence: number;
  langOk: boolean; boundaries: number; boundaryNames: string[];
  expectedCaps: string[]; detectedCaps: string[]; missedCaps: string[]; extraCaps: string[];
}
...
for (const m of manifest) {
  ...
  const detectedCaps = Array.from(new Set(findings.filter(f => f.capability !== 'beacon_telemetry').map(f => f.capability)));
  const missedCaps = m.expect.filter(c => !detectedCaps.includes(c));
  const extraCaps = detectedCaps.filter(c => !m.expect.includes(c));
  results.push({
    file: m.file, expectedLang: m.lang, detectedLang: det.language, langConfidence: det.confidence,
    langOk: det.language === m.lang, boundaries: boundaries.length, boundaryNames: boundaries.map(b=>b.name),
    expectedCaps: m.expect, detectedCaps, missedCaps, extraCaps,
  });
}
...
const totalCapsHit = results.reduce((s, r) => s + r.expectedCaps.filter(c => r.detectedCaps.includes(c)).length, 0);
...
for (const r of results) {
  ...
  byLang[k].files++; if (r.langOk) byLang[k].langOk++;
...
const out = {
  summary: {
    ...
    totalExpectedCaps, totalCapsHit, zeroBoundaryFiles: zeroBoundary,
  },
  byLanguage: Object.fromEntries(Object.entries(byLang).map(([k,v]) => [k, {
    files:v.files, langAccuracy:+(v.langOk/v.files*100).toFixed(1),
    capabilityRecall:v.expected ? +(v.hit/v.expected*100).toFixed(1) : 100,
    expectedCaps:v.expected, hitCaps:v.hit, zeroBoundary:v.zeroBoundary,
  }])),
...
console.log('\nTOP MISSED:');
for (const [c,n] of topMissed) console.log(`  ${c}: missed ×${n}`);
console.log('\nZERO BOUNDARIES:');
for (const r of results.filter(r=>r.boundaries===0)) console.log(`  ${r.file} (det=${r.detectedLang} exp=${r.expectedLang})`);
console.log('\nMISS DETAIL:');
for (const r of results.filter(r=>r.missedCaps.length)) console.log(`  ${r.file}: missed [${r.missedCaps.join(', ')}] | boundaries=${r.boundaryNames.slice(0,6).join(',')}${r.boundaryNames.length>6?'...':''}`);
