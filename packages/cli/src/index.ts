...
// ═══════════════════════════════════════════════════════════════
// Ascension — Polyglot Signal Emitters
// ═══════════════════════════════════════════════════════════════

interface AscensionSignalData {
  project: string;
  fileName: string;
  language: string;
  archetype: string;
  ...
  discoveries: number;
  cjpi: { novelty: number; utility: number; composability: number; maturity: number; total: number; tier: string };
  operator: string;
  fingerprint: string;
  ascendedAt: string;
  collisionDetails: { primitive: string; affinity: number; compatible: boolean }[];
}

function ascensionFingerprint(data: AscensionSignalData): string {
  const { createHash } = require('crypto') as typeof import('crypto');
  const payload = `${data.project}:${data.fileName}:${data.cjpi.total}:${data.collisions}:${data.ascendedAt}`;
  return createHash('sha256').update(payload).digest('hex').slice(0, 16);
}

function ascensionBrandedHeader(d: AscensionSignalData): string {
  const hitPrimitives = d.collisionDetails.filter(c => c.compatible).map(c => c.primitive);
  return [
    '═══════════════════════════════════════════════════════════════════════════════',
    ' CMPSBL® Ascension Receipt — Primitive Collision Report',
    ' ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━',
    '',
    ` Project:      ${d.project}`,
    ` File:         ${d.fileName}`,
    ...
    ` Node ID:      ${d.nodeId}`,
    '',
    ` CJPI Score:   ${d.cjpi.total}/100 (${d.cjpi.tier})`,
    ` Collisions:   ${d.collisions}/${d.totalPrimitives} primitives`,
    ` Discoveries:  ${d.discoveries} unique capability combinations`,
    ` Operator:     ${d.operator}`,
    ` Fingerprint:  ${d.fingerprint}`,
    ` Ascended:     ${d.ascendedAt}`,
    '',
    ` Hit Primitives: ${hitPrimitives.join(', ')}`,
    '',
    ' This file was generated by `cmpsbl ascend`.',
    ' Import it directly — it is a typed constant you can reference in your code.',
    '',
    ' To attach Layer 2 governance: npx mana attach',
    '',
...
function ascensionWrapComment(d: AscensionSignalData, style: 'c' | 'hash' | 'doc'): string {
  const body = ascensionBrandedHeader(d);
  switch (style) {
    case 'c':    return `/*\n${body.split('\n').map(l => ` * ${l}`).join('\n')}\n */`;
    case 'hash': return body.split('\n').map(l => `# ${l}`).join('\n');
    case 'doc':  return body.split('\n').map(l => `//! ${l}`).join('\n');
  }
}

function ascensionSignalBody(d: AscensionSignalData): Record<string, unknown> {
  return {
    active: true,
    project: d.project,
    fileName: d.fileName,
    ...
    totalPrimitives: d.totalPrimitives,
    discoveries: d.discoveries,
    cjpi: d.cjpi,
    operator: d.operator,
    fingerprint: d.fingerprint,
    ascendedAt: d.ascendedAt,
  };
}

function emitAscensionTS(d: AscensionSignalData): string {
  const b = ascensionSignalBody(d);
  return `${ascensionWrapComment(d, 'c')}\n\nexport const ASCENSION_RECEIPT = ${JSON.stringify(b, null, 2)} as const;\n\nexport type AscensionTier = typeof ASCENSION_RECEIPT.cjpi.tier;\n`;
}

function emitAscensionJS(d: AscensionSignalData): string {
  const b = ascensionSignalBody(d);
  return `${ascensionWrapComment(d, 'c')}\n\nexport const ASCENSION_RECEIPT = Object.freeze(${JSON.stringify(b, null, 2)});\n`;
}

function emitAscensionPy(d: AscensionSignalData): string {
  const b = ascensionSignalBody(d);
  const pyBool = (v: boolean) => v ? 'True' : 'False';
  return `"""\n${ascensionBrandedHeader(d)}\n"""\n\nASCENSION_RECEIPT = {\n    "active": ${pyBool(b.active as boolean)},\n    "project": ${JSON.stringify(b.project)},\n    "file_name": ${JSON.stringify(b.fileName)},\n    "language": ${JSON.stringify(b.language)},\n    "archetype": ${JSON.stringify(b.archetype)},\n    "node_id": ${JSON.stringify(b.nodeId)},\n    "collisions": ${b.collisions},\n    "total_primitives": ${b.totalPrimitives},\n    "discoveries": ${b.discoveries},\n    "cjpi": ${JSON.stringify(b.cjpi)},\n    "operator": ${JSON.stringify(b.operator)},\n    "fingerprint": ${JSON.stringify(b.fingerprint)},\n    "ascended_at": ${JSON.stringify(b.ascendedAt)},\n}\n`;
}

function emitAscensionRust(d: AscensionSignalData): string {
  const b = ascensionSignalBody(d);
  return `${ascensionWrapComment(d, 'doc')}\n\npub const ASCENSION_RECEIPT: &str = r#"${JSON.stringify(b, null, 2)}"#;\n`;
}

function emitAscensionGo(d: AscensionSignalData): string {
  const b = ascensionSignalBody(d);
  return `${ascensionWrapComment(d, 'c')}\n\npackage ascension\n\nconst AscensionReceipt = \`${JSON.stringify(b, null, 2)}\`\n`;
}

function emitAscensionGeneric(d: AscensionSignalData): string {
  const b = ascensionSignalBody(d);
  return JSON.stringify({ $schema: 'https://cmpsbl.com/schemas/ascension-receipt-v1.json', ...b }, null, 2) + '\n';
}

interface AscensionEmitter { filename: string; emit: (d: AscensionSignalData) => string }

function getAscensionEmitter(ext: string): AscensionEmitter {
  const base = 'ascension.receipt';
  switch (ext) {
    case '.ts': case '.tsx': return { filename: `${base}.ts`, emit: emitAscensionTS };
    case '.js': case '.jsx': case '.mjs': case '.cjs': return { filename: `${base}.js`, emit: emitAscensionJS };
    case '.py': return { filename: 'ascension_receipt.py', emit: emitAscensionPy };
    case '.rs': return { filename: 'ascension_receipt.rs', emit: emitAscensionRust };
    case '.go': return { filename: 'ascension_receipt.go', emit: emitAscensionGo };
    case '.rb': return { filename: 'ascension_receipt.rb', emit: (d) => `${ascensionWrapComment(d, 'hash')}\n\nASCENSION_RECEIPT = ${JSON.stringify(ascensionSignalBody(d), null, 2)}.freeze\n` };
    case '.php': return { filename: 'ascension_receipt.php', emit: (d) => `<?php\n${ascensionWrapComment(d, 'c')}\n\nreturn ${JSON.stringify(ascensionSignalBody(d), null, 2)};\n` };
    case '.java': case '.kt': return { filename: 'AscensionReceipt.java', emit: (d) => `${ascensionWrapComment(d, 'c')}\npackage ascension;\n\npublic final class AscensionReceipt {\n    public static final String DATA = "${JSON.stringify(ascensionSignalBody(d)).replace(/"/g, '\\"')}";\n    private AscensionReceipt() {}\n}\n` };
    case '.cs': return { filename: 'AscensionReceipt.cs', emit: (d) => `${ascensionWrapComment(d, 'c')}\n\nnamespace Ascension;\n\npublic static class AscensionReceipt\n{\n    public const string Data = @"${JSON.stringify(ascensionSignalBody(d)).replace(/"/g, '""')}";\n}\n` };
    case '.swift': return { filename: 'AscensionReceipt.swift', emit: (d) => `${ascensionWrapComment(d, 'c')}\n\nstruct AscensionReceipt {\n    static let data = #"${JSON.stringify(ascensionSignalBody(d))}"#\n}\n` };
    case '.dart': return { filename: 'ascension_receipt.dart', emit: (d) => `${ascensionWrapComment(d, 'c')}\n\nconst ascensionReceipt = '${JSON.stringify(ascensionSignalBody(d))}';\n` };
    case '.ex': case '.exs': return { filename: 'ascension_receipt.ex', emit: (d) => `${ascensionWrapComment(d, 'hash')}\n\ndefmodule Ascension.Receipt do\n  @receipt ${JSON.stringify(ascensionSignalBody(d), null, 2)}\n  def get, do: @receipt\nend\n` };
    default: return { filename: 'ascension.receipt.json', emit: emitAscensionGeneric };
  }
}

// ═══════════════════════════════════════════════════════════════
// Structural Analysis — Deterministic Affinity Scoring
// ═══════════════════════════════════════════════════════════════
...
function analyzeStructure(code: string): StructuralProfile {
  return {
    imports: (code.match(/import\s+|require\s*\(|from\s+['"]|use\s+|#include/g) ?? []).length,
    exports: (code.match(/export\s+|module\.exports|pub\s+(fn|struct|enum|const|mod)|public\s+(class|func|def)/g) ?? []).length,
    functions: (code.match(/function\s+|fn\s+|def\s+|func\s+|\=>\s*\{|lambda|->|method/g) ?? []).length,
    classes: (code.match(/class\s+|struct\s+|interface\s+|trait\s+|enum\s+|protocol\s+/g) ?? []).length,
    asyncCount: (code.match(/async|await|Promise|Future|Task|goroutine|spawn|tokio|asyncio/gi) ?? []).length,
    errorHandling: (code.match(/try|catch|except|rescue|recover|Result<|Option<|throw|raise|panic/g) ?? []).length,
    stateManagement: (code.match(/useState|setState|store|redux|vuex|atom|signal|observable|mut\s|var\s/g) ?? []).length,
    apiCalls: (code.match(/fetch|axios|http|request|curl|HttpClient|reqwest|net\/http/gi) ?? []).length,
    validation: (code.match(/validate|schema|zod|yup|joi|assert|guard|check|verify/gi) ?? []).length,
    logging: (code.match(/console\.|log\.|logger|println|print\(|debug!|info!|warn!|error!/g) ?? []).length,
    security: (code.match(/auth|token|jwt|encrypt|decrypt|hash|bcrypt|cors|csrf|sanitize|escape/gi) ?? []).length,
    testing: (code.match(/test|spec|describe|it\(|expect|assert|mock|stub|fixture/g) ?? []).length,
    caching: (code.match(/cache|memo|lru|redis|ttl|invalidate|stale/gi) ?? []).length,
    events: (code.match(/emit|on\(|addEventListener|subscribe|publish|dispatch|trigger|observer/g) ?? []).length,
    lineCount: code.split('\n').length,
  };
}

/** Deterministic affinity: each primitive scores based on how relevant the code structure is to its role */
function computePrimitiveAffinity(primitive: typeof PRIMITIVES[0], profile: StructuralProfile): number {
  const total = profile.lineCount;
  if (total === 0) return 0;

  const density = (count: number) => Math.min(count / Math.max(total / 50, 1), 1);

  const roleScores: Record<string, number> = {
    kernel:        0.3 + density(profile.functions) * 0.4 + density(profile.exports) * 0.3,
    runtime:       0.2 + density(profile.asyncCount) * 0.4 + density(profile.functions) * 0.2 + density(profile.events) * 0.2,
    reasoning:     0.1 + density(profile.functions) * 0.3 + density(profile.classes) * 0.3 + density(profile.errorHandling) * 0.3,
    persistence:   density(profile.stateManagement) * 0.5 + density(profile.caching) * 0.3 + density(profile.imports) * 0.2,
    signaling:     density(profile.events) * 0.5 + density(profile.asyncCount) * 0.3 + density(profile.logging) * 0.2,
    intelligence:  density(profile.functions) * 0.3 + density(profile.apiCalls) * 0.3 + density(profile.asyncCount) * 0.2 + 0.2,
    identity:      density(profile.security) * 0.6 + density(profile.validation) * 0.3 + 0.1,
    sovereignty:   density(profile.security) * 0.4 + density(profile.errorHandling) * 0.3 + density(profile.validation) * 0.3,
    mapping:       density(profile.imports) * 0.4 + density(profile.exports) * 0.3 + density(profile.classes) * 0.3,
    healing:       density(profile.errorHandling) * 0.5 + density(profile.logging) * 0.3 + density(profile.testing) * 0.2,
    delivery:      density(profile.apiCalls) * 0.4 + density(profile.events) * 0.3 + density(profile.asyncCount) * 0.3,
    ethics:        density(profile.validation) * 0.4 + density(profile.security) * 0.3 + density(profile.errorHandling) * 0.3,
    protection:    density(profile.security) * 0.5 + density(profile.errorHandling) * 0.3 + density(profile.validation) * 0.2,
    anomaly:       density(profile.errorHandling) * 0.4 + density(profile.logging) * 0.3 + density(profile.testing) * 0.3,
    policy:        density(profile.validation) * 0.4 + density(profile.security) * 0.3 + density(profile.errorHandling) * 0.3,
    trust:         density(profile.security) * 0.4 + density(profile.validation) * 0.3 + density(profile.apiCalls) * 0.3,
    adaptation:    density(profile.stateManagement) * 0.4 + density(profile.events) * 0.3 + density(profile.asyncCount) * 0.3,
    reaction:      density(profile.events) * 0.5 + density(profile.asyncCount) * 0.3 + density(profile.errorHandling) * 0.2,
    navigation:    density(profile.imports) * 0.4 + density(profile.classes) * 0.3 + density(profile.functions) * 0.3,
    connectors:    density(profile.apiCalls) * 0.5 + density(profile.imports) * 0.3 + density(profile.asyncCount) * 0.2,
    resolution:    density(profile.functions) * 0.4 + density(profile.validation) * 0.3 + density(profile.errorHandling) * 0.3,
    auth:          density(profile.security) * 0.6 + density(profile.validation) * 0.2 + density(profile.apiCalls) * 0.2,
    observability: density(profile.logging) * 0.5 + density(profile.events) * 0.3 + density(profile.errorHandling) * 0.2,
    stealth:       density(profile.security) * 0.3 + density(profile.errorHandling) * 0.3 + 0.1 + density(profile.caching) * 0.3,
    synthesis:     density(profile.functions) * 0.3 + density(profile.classes) * 0.3 + density(profile.asyncCount) * 0.2 + 0.2,
    extraction:    density(profile.imports) * 0.3 + density(profile.apiCalls) * 0.3 + density(profile.functions) * 0.2 + 0.2,
    fabrication:   density(profile.exports) * 0.4 + density(profile.classes) * 0.3 + density(profile.functions) * 0.3,
    language:      density(profile.validation) * 0.3 + density(profile.functions) * 0.3 + density(profile.logging) * 0.2 + 0.2,
    reflection:    density(profile.logging) * 0.4 + density(profile.testing) * 0.3 + density(profile.errorHandling) * 0.3,
    speculation:   density(profile.asyncCount) * 0.3 + density(profile.apiCalls) * 0.3 + density(profile.stateManagement) * 0.2 + 0.2,
    isolation:     density(profile.errorHandling) * 0.4 + density(profile.testing) * 0.3 + density(profile.security) * 0.3,
    messaging:     density(profile.events) * 0.5 + density(profile.asyncCount) * 0.3 + density(profile.apiCalls) * 0.2,
    generation:    density(profile.exports) * 0.4 + density(profile.functions) * 0.3 + density(profile.classes) * 0.3,
    analysis:      density(profile.functions) * 0.3 + density(profile.validation) * 0.3 + density(profile.testing) * 0.2 + 0.2,
    compliance:    density(profile.validation) * 0.4 + density(profile.security) * 0.3 + density(profile.logging) * 0.3,
    metering:      density(profile.logging) * 0.4 + density(profile.apiCalls) * 0.3 + density(profile.caching) * 0.3,
    accessibility: density(profile.classes) * 0.3 + density(profile.validation) * 0.3 + density(profile.logging) * 0.2 + 0.2,
    orchestration: density(profile.asyncCount) * 0.3 + density(profile.functions) * 0.3 + density(profile.events) * 0.2 + 0.2,
    prediction:    density(profile.apiCalls) * 0.3 + density(profile.stateManagement) * 0.3 + density(profile.asyncCount) * 0.2 + 0.2,
    infrastructure:density(profile.imports) * 0.3 + density(profile.apiCalls) * 0.3 + density(profile.errorHandling) * 0.2 + 0.2,
  };

  const score = roleScores[primitive.role] ?? 0.25;
  return Math.min(Math.round(score * 100) / 100, 1.0);
}

/** Deterministic CJPI from structural profile — no randomness */
function computeStructuralCJPI(profile: StructuralProfile, collisions: number, totalPrimitives: number): { novelty: number; utility: number; composability: number; maturity: number; total: number; tier: string } {
  const collisionRatio = collisions / totalPrimitives;

  const novelty = Math.min(Math.round(
    (profile.asyncCount > 3 ? 8 : 3) +
    (profile.events > 2 ? 6 : 2) +
    (profile.security > 2 ? 5 : 1) +
    collisionRatio * 6
  ), 25);

  const utility = Math.min(Math.round(
    (profile.functions > 3 ? 7 : 3) +
    (profile.exports > 2 ? 5 : 2) +
    (profile.errorHandling > 2 ? 5 : 1) +
    (profile.apiCalls > 0 ? 4 : 1) +
    collisionRatio * 4
  ), 25);

  const composability = Math.min(Math.round(
    (profile.classes > 1 ? 6 : 2) +
    (profile.imports > 3 ? 5 : 2) +
    (profile.validation > 1 ? 4 : 1) +
    collisionRatio * 5 +
    (profile.events > 0 ? 3 : 0)
  ), 25);

  const maturity = Math.min(Math.round(
    (profile.testing > 0 ? 6 : 1) +
    (profile.logging > 2 ? 4 : 1) +
    (profile.errorHandling > 3 ? 5 : 2) +
    (profile.lineCount > 100 ? 4 : profile.lineCount > 50 ? 2 : 1) +
    collisionRatio * 3
  ), 25);
  ...
  return { novelty, utility, composability, maturity, total, tier };
}
...
async function cmdAscend(args: string[]): Promise<void> {
  ...
  // Stage 1: Upload + Structural Analysis
  const s1 = spinner('Stage 1/8 — UPLOAD: Extracting structural profile...');
  await sleep(800);
  const profile = analyzeStructure(fileContent);
  s1.stop(`Stage 1/8 — UPLOAD: ${profile.imports} imports, ${profile.exports} exports, ${profile.functions} functions, ${profile.classes} classes`);
  ...
  // Stage 3: Register as Primitive #41
  ...
  // Stage 4: Chain (deterministic collision against 40 primitives)
  ...
  for (const prim of PRIMITIVES) {
    const affinity = computePrimitiveAffinity(prim, profile);
    const compatible = affinity > 0.30;
    collisionResults.push({ primitive: prim.id, affinity, compatible });
    if (supportsAnimatedOutput()) {
      const icon = compatible ? c.green('⚡') : c.dim('·');
      const affinityStr = (affinity * 100).toFixed(0).padStart(3) + '%';
      process.stdout.write(`\r    ${icon} ${prim.id.padEnd(14)} ${compatible ? c.green('COLLISION') : c.dim('pass')} ${c.dim(affinityStr)}    `);
      await sleep(80);
    }
  }
  ...
  // Stage 5: Discover (capability combinations from collisions)
  const s5 = spinner('Stage 5/8 — DISCOVER: Mapping unique capability combinations...');
  await sleep(1200);
  const hitPrimitives = collisionResults.filter(r => r.compatible);
  const discoveries = Math.min(hitPrimitives.length * (hitPrimitives.length - 1) / 2, hitPrimitives.length * 3);
  s5.stop(`Stage 5/8 — DISCOVER: ${c.bold(String(Math.round(discoveries)))} unique capability combinations found`);

  // Stage 6: Score (Deterministic CJPI)
  const s6 = spinner('Stage 6/8 — SCORE: Computing CJPI tier assignment...');
  await sleep(1000);
  const cjpi = computeStructuralCJPI(profile, hits, PRIMITIVES.length);
  ...
  const operatorName = storedCreds?.displayName ?? os.userInfo().username ?? 'Operator';
  const now = new Date().toISOString();

  const signalData: AscensionSignalData = {
    ...
    fingerprint: '',
    ascendedAt: now,
    collisionDetails: collisionResults,
  };
  signalData.fingerprint = ascensionFingerprint(signalData);

  const emitter = getAscensionEmitter(fileExt);
  const outputContent = emitter.emit(signalData);
  const outputPath = path.join(process.cwd(), emitter.filename);
  fs.writeFileSync(outputPath, outputContent);

  s7.stop(`Stage 7/8 — EXPORT: ${c.bold(emitter.filename)} generated`);

  // Stage 8: Protect (integrity seal)
  const s8 = spinner('Stage 8/8 — PROTECT: Applying integrity seal + fingerprint...');
  await sleep(600);
  s8.stop(`Stage 8/8 — PROTECT: Fingerprint ${c.cyan(signalData.fingerprint)}`);
  ...
  say(`    ${c.dim('Discoveries:')}    ${Math.round(discoveries)} unique combinations`);
  say(`    ${c.dim('CJPI Score:')}     ${tierColor(`${cjpi.total}/100 ${cjpi.tier}`)}`);
  say(`    ${c.dim('  Novelty:')}      ${progressBar(cjpi.novelty, 25, 15)} ${cjpi.novelty}`);
  say(`    ${c.dim('  Utility:')}      ${progressBar(cjpi.utility, 25, 15)} ${cjpi.utility}`);
