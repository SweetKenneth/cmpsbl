...
const FAQ_ITEMS = [
  ...
  { question: "How does the Proxy wrapping actually work?", answer: "When Mana attaches to a host package, it replaces each target export with a JavaScript Proxy. The Proxy's 'apply' trap intercepts every call: it runs Lex governance checks, logs telemetry, enforces shadow rules, and then calls the original function unchanged. The caller sees no difference — same arguments in, same result out — but Layer 2 is active around every invocation. The original function reference is stored internally for clean detachment." },
  ...
  { question: "What can I attach with Mana?", answer: "Anything that runs as a JavaScript/TypeScript package. Mana scans host exports and wraps them with Proxy-based attachment points. Current capabilities include: DEFENSE gates (block exploit paths), BEACON telemetry (observe usage patterns), Shadow Rules (governance overrides), Circuit Breakers (fault isolation), Governance Hooks (policy enforcement), and Audit Trails (immutable logging)." },
  { question: "What's the difference between Ascension and Mana?", answer: "Ascension is the scanning and diagnosis engine — it discovers what your code needs by running it through the 40-primitive matrix, scoring it via CJPI, and producing a findings manifest. Mana is the deployment and defense engine — it takes those findings and silently attaches Layer 2 capabilities to your software using Proxy-based wrapping, governed by Lex. Ascension answers 'what does this code need?' Mana answers 'how do we deploy it — without permission or source modification?' Two patents, two engines, one pipeline." },
              ...
              {[
                {
                  step: "01",
                  title: "Install & Scan",
                  desc: "Install the openai npm package on our server. Mana scans the package, discovering chat.completions.create, embeddings.create, and 40+ exported functions.",
