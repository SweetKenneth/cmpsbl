...
export default function WhenOneModelIsntEnough() {
  return (
    <BlogArticleLayout slug={SLUG} title="When One Model Isn't Enough" subtitle="Multi-model chains that match complexity to capability" date="March 25, 2025" readTime="16 min read" heroImage={heroImg} heroAlt="CASCADE adaptive intelligence system" chapter={21} head={<><SEO title="When One Model Isn't Enough — The CASCADE Node" description="A single LLM can't handle everything. CASCADE chains multiple models into adaptive processes that match complexity to capability." type="article" image={heroImg} publishedTime="2025-03-25" keywords={["CASCADE node", "multi-model chains", "adaptive AI"]} canonical={`https://cmpsbl.com/blog/${SLUG}`} topicCluster="substrate-nodes" breadcrumbs={[{ name: "Home", url: "https://cmpsbl.com" }, { name: "Blog", url: "https://cmpsbl.com/blog" }, { name: "When One Model Isn't Enough", url: `https://cmpsbl.com/blog/${SLUG}` }]} /><BlogArticleJsonLd title="When One Model Isn't Enough — The CASCADE Node" description="Multi-model chains that match complexity to capability." slug={SLUG} datePublished="2025-03-25" imageUrl={heroImg} keywords={["CASCADE", "multi-model", "adaptive"]} /></>}>
      <p className="text-lg leading-relaxed">By late March, <Link to="/blog/routing-the-unknown" className="text-primary hover:underline">NEXUS</Link> was routing requests to the best single model. But some problems don't have a "best single model." They need a chain.</p>
      ...
      <p>The key insight was that pipeline stages aren't just sequential — they can branch and merge. A document analysis might fork into parallel comprehension and entity extraction paths, then merge at a synthesis stage. This directed acyclic graph approach gives CASCADE the flexibility to handle workflows that linear pipelines can't express. We spent two weeks getting the merge semantics right — how do you combine outputs from two models that disagree? — and the solution involved confidence-weighted averaging that later became a core primitive across the substrate.</p>
      ...
      <p>CASCADE maintains a capability matrix — a scored grid of which models perform best at which tasks. This matrix isn't static. Every pipeline execution updates the scores based on actual output quality. A model that underperforms on entity extraction in legal documents gets deprioritized for that specific stage. Over time, CASCADE builds a highly specialized understanding of model strengths that goes far beyond published benchmarks.</p>
      <p>We also discovered that model version updates could silently break pipelines. When a provider updates their model, the capability profile shifts — sometimes subtly, sometimes dramatically. CASCADE now runs shadow pipelines on model updates, comparing outputs to the previous version before switching production traffic. This canary system caught three regressions in Q2 2025 alone that would have degraded customer-facing outputs.</p>

      <h2 className="text-2xl font-bold text-foreground mt-8">The Cost Equation</h2>
      <p>Multi-model pipelines sound expensive. In practice, they're cheaper than single-model approaches for complex tasks. A $0.01/token model handling a task it takes 2000 tokens to complete poorly costs more than a three-stage pipeline using $0.001, $0.005, and $0.002/token models that complete the same task in 800 total tokens with higher quality. CASCADE optimizes for cost-adjusted quality — the best output per dollar spent.</p>
