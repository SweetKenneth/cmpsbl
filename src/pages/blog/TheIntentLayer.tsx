...
export default function TheIntentLayer() {
  return (
    <BlogArticleLayout slug={SLUG} title="The Intent Layer" subtitle="Natural language to substrate operations via the Intent Mesh" date="November 20, 2025" readTime="17 min read" heroImage={heroImg} heroAlt="Intent Mesh routing natural language to substrate primitives" chapter={31} head={<><SEO title="The Intent Layer — Intent Mesh" description="The Intent Mesh translates natural language into substrate operations across any number of primitives." type="article" image={heroImg} publishedTime="2025-11-20" keywords={["Intent Mesh", "natural language routing", "semantic routing"]} canonical={`https://cmpsbl.com/blog/${SLUG}`} topicCluster="substrate-primitives" breadcrumbs={[{ name: "Home", url: "https://cmpsbl.com" }, { name: "Blog", url: "https://cmpsbl.com/blog" }, { name: "The Intent Layer", url: `https://cmpsbl.com/blog/${SLUG}` }]} /><BlogArticleJsonLd title="The Intent Layer — Intent Mesh" description="Natural language to substrate operations." slug={SLUG} datePublished="2025-11-20" imageUrl={heroImg} keywords={["Intent Mesh", "NLU", "routing"]} /></>}>
      <p className="text-sm sm:text-base leading-relaxed">"I want to know if my website is secure and accessible." That's a sentence a human says. What the substrate hears is: invoke <Link to="/blog/when-bots-found-us-first" className="text-primary hover:underline">DEFENSE</Link> for security scanning, invoke <Link to="/blog/accessibility-is-infrastructure" className="text-primary hover:underline">INCLUSIVE</Link> for accessibility auditing, merge results through <Link to="/blog/breaking-problems-apart" className="text-primary hover:underline">DECODE</Link>, and format via <Link to="/blog/from-thought-to-artifact" className="text-primary hover:underline">FORGE</Link>. The Intent Mesh bridges that gap.</p>
      <p>Before the Intent Mesh, using the substrate required knowing its vocabulary. Developers had to specify which primitives to invoke, what parameters to pass, and how to compose operations. It was powerful but opaque — like writing SQL when you want to ask a question. The Intent Mesh is the natural language interface that turns questions into queries.</p>

      <h2 className="text-2xl font-bold text-foreground mt-8">Intent Classification</h2>
      <p>The mesh classifies user intent at multiple levels. What domain? (security, accessibility, content). What operation? (analyze, generate, monitor). What scope? (one page, whole site, ongoing). Each classification narrows which primitives participate and how they interact.</p>
      ...
      <p>Simple intents map to single nodes. Complex intents fan out across the substrate. The Intent Mesh builds an execution plan — a directed graph of primitive invocations with data dependencies — and executes it through <Link to="/blog/nodes-that-talk" className="text-primary hover:underline">RIPPLE</Link>. The user sees one response. The substrate performed twelve operations.</p>
      <p>The execution planner is the most sophisticated part of the Intent Mesh. It considers node capabilities, current load, cost, and expected quality when building plans. A plan for "analyze my competitor's website" might involve DEFENSE for technical analysis, BRAIN for historical context from previous analyses, DECODE for structural decomposition, CORTEX for multi-model synthesis, AUDIT for validation, and FORGE for report generation — all orchestrated in the correct dependency order with parallel execution where possible. Plan construction takes about 200ms. Plan execution takes seconds to minutes depending on complexity.</p>

      <h2 className="text-2xl font-bold text-foreground mt-8">Learning From Misses</h2>
      <p>When users rephrase or retry, the mesh learns. "That's not what I meant" signals are gold. Every correction refines the intent classification model. Over time, the mesh anticipates what users want before they finish typing — the same pattern that <Link to="/blog/interfaces-that-think" className="text-primary hover:underline">FORGE</Link> uses to pre-render interfaces.</p>
