...
export default function WhatWeGotWrong() {
  return (
    <BlogArticleLayout slug={SLUG} title="What We Got Wrong" subtitle="Over-engineered primitives, premature abstractions, and lessons learned" date="February 22, 2026" readTime="19 min read" heroImage={heroImg} heroAlt="Engineering retrospective" chapter={38} head={<><SEO title="What We Got Wrong" description="Over-engineered primitives, premature abstractions, and architectural decisions we'd reverse if we could." type="article" image={heroImg} publishedTime="2026-02-22" keywords={["engineering mistakes", "technical retrospective", "lessons learned"]} canonical={`https://cmpsbl.com/blog/${SLUG}`} topicCluster="substrate-retrospective" breadcrumbs={[{ name: "Home", url: "https://cmpsbl.com" }, { name: "Blog", url: "https://cmpsbl.com/blog" }, { name: "What We Got Wrong", url: `https://cmpsbl.com/blog/${SLUG}` }]} /><BlogArticleJsonLd title="What We Got Wrong" description="Honest retrospective on mistakes and lessons." slug={SLUG} datePublished="2026-02-22" imageUrl={heroImg} keywords={["mistakes", "retrospective", "lessons"]} /></>}>
      <p className="text-sm sm:text-base leading-relaxed">Forty chapters of building the substrate. This is the one where we're honest about what didn't work. Every origin story has failures — ours is no exception.</p>
      ...
      <p>The counter-argument is that some infrastructure needs to exist before customers ask for it — you can't retrofit observability into a system that wasn't built for it. That's true. But we built AUDIT to handle regulatory frameworks we hadn't encountered yet, RIPPLE to support event volumes we wouldn't reach for six months, and ATLAS to map dependencies between primitives that didn't exist. The useful abstraction boundary is "build for the next known problem, not the third." We consistently built for the third, and some of that investment was wasted on capabilities we never used in the form we originally designed.</p>

      <h2 className="text-2xl font-bold text-foreground mt-8">Trying to Do Everything</h2>
      <p>Forty primitives is a lot. Some of them overlap. Some exist because we thought we'd need them, not because users asked for them. The honest truth is that maybe 15-20 primitives carry 95% of the value. The rest are edge cases that could have waited. But they're built now, and removing them would break more than it fixes.</p>
      <p>We did a usage analysis in January. Five primitives (NEXUS, BRAIN, CORTEX, DECODE, DEFENSE) handle 72% of all substrate traffic. The next five handle 18%. The remaining thirty primitives share the last 10%. Some of those thirty primitives serve critical but infrequent functions — AUDIT fires rarely but is essential when it does. Others are genuinely underused. Three primitives had fewer than 100 invocations in all of January. We're keeping them because the removal cost exceeds the maintenance cost, but we wouldn't build them again.</p>
