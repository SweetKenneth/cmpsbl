...
export default function TrustButVerify() {
  return (
    <BlogArticleLayout slug={SLUG} title="Trust But Verify" subtitle="Output validation, hallucination detection, and the AUDIT Agent" date="May 18, 2025" readTime="17 min read" heroImage={heroImg} heroAlt="AUDIT Agent validating AI outputs" chapter={23} head={<><SEO title="Trust But Verify — The AUDIT Agent" description="AI outputs are probabilistic. AUDIT validates them against constraints, catches hallucinations, and ensures quality thresholds." type="article" image={heroImg} publishedTime="2025-05-18" keywords={["AUDIT Agent", "AI validation", "hallucination detection"]} canonical={`https://cmpsbl.com/blog/${SLUG}`} topicCluster="substrate-primitives" breadcrumbs={[{ name: "Home", url: "https://cmpsbl.com" }, { name: "Blog", url: "https://cmpsbl.com/blog" }, { name: "Trust But Verify", url: `https://cmpsbl.com/blog/${SLUG}` }]} /><BlogArticleJsonLd title="Trust But Verify — The AUDIT Agent" description="Output validation and hallucination detection." slug={SLUG} datePublished="2025-05-18" imageUrl={heroImg} keywords={["AUDIT", "validation", "hallucination"]} /></>}>
      <p className="text-sm sm:text-base leading-relaxed">In May, a customer's chatbot confidently told a user their refund had been processed. It hadn't. The model hallucinated a transaction ID. That was the week we built AUDIT.</p>
      ...
      <p>AUDIT validates AI outputs against developer-defined constraints. "The response must reference only existing product IDs." "Code output must parse without syntax errors." "Numerical claims must be within 5% of source data." These aren't vibes — they're executable validation rules.</p>
      ...
      <p>When AUDIT rejects an output, it doesn't just say "try again." It provides structured feedback to the generation process about what failed. "The third paragraph references a non-existent case study. Regenerate with this constraint: all case studies must come from the following verified list." This targeted re-generation produces better second attempts than naive retries. Our data shows that targeted re-generation succeeds on the first retry 78% of the time, compared to 31% for blind retries.</p>
      <p>AUDIT didn't eliminate hallucinations. Nothing does. But it dropped our hallucination-to-user rate by 87%. Later, <Link to="/blog/the-governance-question" className="text-primary hover:underline">AUDIT</Link> used AUDIT scores as part of its compliance evidence chain, and the constraint system became a selling point for regulated industries where output accuracy isn't optional — it's legally required.</p>
    </BlogArticleLayout>
  );
}
