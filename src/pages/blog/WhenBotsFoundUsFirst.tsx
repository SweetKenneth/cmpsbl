...
export default function WhenBotsFoundUsFirst() {
  return (
    <BlogArticleLayout slug={SLUG} title="When Bots Found Us First" subtitle="How automated attacks forced us to build DEFENSE" date="March 5, 2025" readTime="14 min read" heroImage={heroImg} heroAlt="DEFENSE layer security architecture" chapter={4} showRewrittenNotice={false} head={<><SEO title="When Bots Found Us First — Building DEFENSE" description="We didn't plan to build a security node. Then automated attacks found our API endpoints before we'd even launched." type="article" image={heroImg} publishedTime="2025-03-05" keywords={["AI security defense", "DEFENSE layer", "bot attack defense", "behavioral fingerprinting"]} canonical={`https://cmpsbl.com/blog/${SLUG}`} topicCluster="substrate-security" breadcrumbs={[{ name: "Home", url: "https://cmpsbl.com" }, { name: "Blog", url: "https://cmpsbl.com/blog" }, { name: "When Bots Found Us First", url: `https://cmpsbl.com/blog/${SLUG}` }]} /><BlogArticleJsonLd title="When Bots Found Us First — Building DEFENSE" description="How automated attacks forced us to build DEFENSE before launch." slug={SLUG} datePublished="2025-03-05" imageUrl={heroImg} keywords={["DEFENSE layer", "bot defense", "AI security"]} /></>}>
      ...
      <p>We started logging everything: request timing (millisecond precision), payload entropy, session coherence, endpoint access patterns, typing speed on our dashboard, mouse movement trajectories, scroll behavior. Humans are messy and unpredictable. Bots, even sophisticated ones, have <a href="https://en.wikipedia.org/wiki/Device_fingerprint" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline"rel="noopener noreferrer">statistical fingerprints</a> that emerge over a sequence of 10-20 requests.</p>
      ...
      <p>The second class of attack was <a href="https://owasp.org/www-project-top-10-for-large-language-model-applications/" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline"rel="noopener noreferrer">prompt injection</a>. Requests designed to manipulate the AI models behind our API — attempts to extract system prompts, bypass safety filters, or coerce models into generating harmful content. These attacks were clever: payloads encoded in base64, split across multiple parameters, or hidden in seemingly innocuous text.</p>
