...
const SDK_TEMPLATES = [
  ...
  { id: 'defense-scanner', name: 'DEFENSE Scanner', description: 'Anomaly detection engine', category: 'defense', tier: 'free' as const, code: `import { DefenseClient } from '@cmpsbl/defense';\n\nconst defense = new DefenseClient();\n\n// Scan for anomalies\nconst report = await defense.scan({\n  target: 'api-endpoint',\n  depth: 'standard'\n});\n\nconsole.log(report.threats);\nconsole.log(report.score); // 0.0 - 1.0` },
  ...
  { id: 'pipeline-export', name: 'Memory Exporter', description: 'Export crystallized memories to code', category: 'build', tier: 'creator' as const, code: `import { PipelineExporter } from '@cmpsbl/foundry';\n\nconst exporter = new PipelineExporter();\n\n// Export a crystallized memory to Python\nconst artifact = await exporter.export({\n  pipelineId: 'pipe_abc123',\n  language: 'python',\n  includeRuntime: true\n});\n\nconsole.log(artifact.code);\nconsole.log(artifact.testHarness);` },
