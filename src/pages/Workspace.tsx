...
const SDK_TEMPLATES = [
  ...
  { id: 'sdk-discover', name: 'Memory Stream Discovery', description: 'Discover new pipelines from system behavior', category: 'discovery', tier: 'free' as const, code: `import { CMPSBL } from '@cmpsbl/sdk';\n\nconst cmpsbl = new CMPSBL({ apiKey: 'your-api-key' });\n\n// Discovery starts automatically\nconst discovery = await cmpsbl.discover({\n  input: 'track user behavior across sessions'\n});\n\nif (discovery.detected) {\n  console.log(discovery.memory);\n  await cmpsbl.capture(discovery.memory.id);\n}` },
  { id: 'runtime-cjpi', name: 'CJPI Scoring', description: 'Pipeline scoring and auto-tiering via mini runtime', category: 'runtime', tier: 'creator' as const, code: `import { score, classify } from '@cmpsbl/runtime';\n\n// Score a discovered pipeline\nconst cjpi = score({\n  novelty: 0.82,\n  utility: 0.91,\n  complexity: 0.65,\n  composability: 0.78\n});\n\n// Auto-classify tier\nconst tier = classify(cjpi);\nconsole.log(tier); // 'Prime' | 'Relic' | 'Mythic' | 'Apex'` },
