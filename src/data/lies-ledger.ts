...
export const LIES_LEDGER: Finding[] = [
  ...
  {
    id: 'F-113',
    title: 'Terminal Commands — RUNTIME mutation audit: dream.feed and brain.reflect actually mutate persisted state (not read-only theater); evolution.propose is broken with a code defect',
    severity: 'PARTIAL',
    source: {
      document: 'supabase/functions/pf-substrate/index.ts:12689 (dream.feed) + :8670 (brain.reflect) + :14945 (evolution.propose)',
      quote: 'Marketing claim: terminal commands "make the system dream, evolve, learn." Investor claim: "Every user gets persistent memory, Memory Stream access … and full module telemetry at $0."',
    },
    evidence: {
      reality:
        'Live before/after DB diffing 2026-04-18 03:43 UTC. Methodology: capture DB row state, fire one terminal-equivalent edge POST, capture state again, diff. (1) DREAM.FEED — POST {module:"dream",action:"feed",data:{input:"audit-test-payload-2026-04-18",source:"audit"}} returned HTTP 200 + a real dream record. Direct psql verified: cascade_dreams now contains row id=6ca1e911 with dream_text="audit-test-payload-2026-04-18", mood="dreaming", created_at=2026-04-18 03:43:49.855Z — written by THIS test, did not exist before. The dream_eater_state row updated atomically: mood_score moved from 0.10 → 0.20 (the +0.10 boost the handler claims), last_fed_at advanced from 2026-03-03 to 2026-04-18 03:43:49.93Z, dreams_consumed_today went from 0 to 9 (counter incremented through normal feed processing). This is REAL state mutation, not a no-op. (2) BRAIN.REFLECT — POST {module:"brain",action:"reflect",data:{depth:1}} returned HTTP 200 + reflection id=fcc5c03b. Direct psql verified: brain_reflections now contains row id=fcc5c03b with reflection_date=2026-04-18 and summary "Analyzed 50 memories. Top patterns: defense_monitoring, brain_learning_activity, api_usage_pattern" — content reflects the ACTUAL state of brain_memories (analyzed real rows, named real patterns from learning_patterns table). The substrate truly reflects on its own data. (3) EVOLUTION.PROPOSE — POST {module:"evolution",action:"propose",data:{scope:"all",notes:"audit"}} returned HTTP 500: "supabase.from(...).insert(...).catch is not a function" — this is a real JavaScript error, not a permissions issue. The handler at index.ts:14945 chains `.catch()` to a Supabase insert builder, but the Supabase JS client returns a Promise-like object that lacks `.catch` until awaited. evolution_proposals row count stayed at 34 before AND after — proposal was NOT created. The 34 rows ARE real (verified F-109), but no NEW proposal can currently be generated through the public terminal command — the substrate is frozen at 34. (4) DREAM.CONSUME — returned 400 "dream_id is required", which is a documentation gap not a defect (the consume action targets a specific dream id, not a free-form "consume next" cycle as the terminal example implies).',
      method: 'psql before/after diffing on cascade_dreams, dream_eater_state, brain_reflections, evolution_proposals; live HTTPS POST to /functions/v1/pf-substrate; verbatim error capture from edge response.',
    },
    verdict:
      'Major finding splits in two directions. POSITIVE: dream.feed and brain.reflect are THE OPPOSITE of theater — they perform real, atomic, multi-table state mutations exactly as advertised (insert dream + update mood_score + advance last_fed_at + bump counter, all in one call). The substrate genuinely consumes user input, classifies it, persists it, and updates its emotional state. brain.reflect genuinely reads 50 memories, identifies real patterns, and writes a real daily reflection row. The "make the system dream" claim is LITERALLY TRUE for these commands. NEGATIVE: evolution.propose has a runtime defect (`.catch is not a function` on a Supabase builder) that means no new evolution proposals can be generated through the terminal — the 34 existing proposals are real historical artifacts, but the EVOLUTION engine cannot currently produce a 35th via the public path. Fix: change `.insert(…).catch(e => …)` to `await .insert(…)` with try/catch. One-line patch converts a "broken evolve" claim back into a working one. Net assessment: the system genuinely dreams and reflects, but currently does not evolve through the terminal.',
    recordedAt: '2026-04-18',
  },
  {
    id: 'F-114',
    title: 'QUANTUM™ Crown Jewels — 224 entries marked `hasCode: true` are 100% metadata records with ZERO executable algorithms (only ONE real 164-LOC implementation file exists for the entire vertical)',
    severity: 'THEATER',
    source: {
      document: 'src/crownjewels/quantum-vertical-registry.ts (header) + src/crownjewels/a-tier/quantum-atier.ts (header) + the cj() factory function in both files',
      quote: '"144 Architectural Crown Jewels: 9 per each of the 16 quantum primitives" + "80 A-Tier Crown Jewels: 5 per each of the 16 quantum primitives. Governor-curated, Architecture-class." Both files set `hasCode: true` on every record. Marketing surface lists Quantum™ as a fully operational 16-primitive vertical at quantum.cmpsbl.com.',
    },
    evidence: {
      reality:
        'Live audit 2026-04-18. (1) ENTRY COUNT — `grep -c "^  cj(" src/crownjewels/quantum-vertical-registry.ts` returns 144 records (S-tier, CJPI 86–98); same grep on a-tier/quantum-atier.ts returns 80 (A-tier, CJPI 85–91). 224 total quantum Crown Jewels claimed. Every single one is constructed via the same `cj(rank, id, name, cjpi, module, description, sig)` factory and hard-codes `hasCode: true` at line 27 of quantum-atier.ts. (2) WHAT cj() PRODUCES — a STierEntry object containing: rank, id (e.g. "S-QBT01"), name (e.g. "Quantum Circuit Transpiler"), cjpi, module (e.g. "QUBIT"), description (single sentence prose), signatureHash, version "1.0.0", approved: true, generatedAt, hasCode: true. NO executable function, NO algorithm, NO computation, NO inputs, NO outputs. Pure metadata. (3) ACTUAL EXECUTABLE QUANTUM CODE in src/ — one file: src/lib/export/layers/inventory/quantum-simulation-suite.layer.ts (164 LOC) implements four real deterministic primitives: cmpsbl_qss_hadron_seed (Mulberry32 PRNG, ~10 LOC), cmpsbl_qss_qubit_h/x/measure (single-qubit Hadamard/Pauli-X/Born-rule measurement, ~15 LOC), cmpsbl_qss_entangle_bell (Φ+ Bell-pair correlated measurement, ~5 LOC), cmpsbl_qss_lattice_step (2-D heat-equation diffusion kernel, ~25 LOC). That is the ENTIRE working quantum surface. It covers 4 of the 16 advertised quantum primitives (HADRON, QUBIT, ENTANGLE, LATTICE) at the most introductory textbook level — single-qubit only, no multi-qubit gates beyond Bell-pair, no transpilation, no error correction, no VQE, no QAOA, no surface-code decoding, no QKD, no plasma MHD, no cryogenic modeling. None of the 224 named Crown Jewels (e.g. "Variational Ansatz Optimizer", "Error Syndrome Decoder", "Quantum Volume Calculator", "Hong-Ou-Mandel Interference Engine", "Configuration Interaction Solver", "Schmidt Decomposition Engine", "Quantum Tunneling Rate Estimator") have implementations anywhere in src/. (4) FACTORY VERTICAL — src/lib/factory/verticals/quantum.ts (456 LOC) declares 16 VerticalPrimitive objects with .capabilities arrays of strings ("particle_collision_sim", "gate_synthesis", "boson_sampling_sim", etc.). These are LABELS, not functions — there is no executor that maps a capability string to a real algorithm. (5) PUBLIC SURFACE — src/pages/QuantumHome.tsx exists (242 LOC), quantum.cmpsbl.com is a custom domain, and src/assets contains marketing images "quantum-simulation-suite.jpg" and "quantum-error-correction-engine.jpg" — but the engine in those images does not exist as code. (6) NO QUANTUM HARDWARE BRIDGE — no Qiskit, no Cirq, no PyQuil, no Braket SDK, no QuTiP wrapper, no IBM Q backend client, no D-Wave client. The entire Quantum vertical can compute exactly what the 164 LOC of quantum-simulation-suite.layer.ts can compute: a deterministic PRNG, single-qubit Hadamard/X, one Bell-pair correlation, one heat diffusion step. That is roughly equivalent to the first homework assignment in an introductory quantum computing course.',
      method: 'find src -iname "*quantum*"; grep -c "^  cj(" on both registry files; wc -l + head on quantum-simulation-suite.layer.ts and quantum.ts; grep for actual function declarations / class definitions / algorithm implementations across src/; verified absence of any quantum SDK in package.json.',
    },
    verdict:
      'The Quantum™ vertical is the largest theater finding in the ledger to date — larger by surface area than the depot façade (F-112). 224 Crown Jewels with `hasCode: true` and zero code, an entire 16-primitive vertical with one toy 164-LOC implementation file, a custom subdomain (quantum.cmpsbl.com), a marketing landing page, marketplace artwork — all backed by what amounts to a single page of intro-level quantum simulation code. The CJPI scores (86–98) imply enterprise-grade IP value; reality is a textbook exercise. Two honest framings are possible: (a) downgrade `hasCode: false` on all 224 entries and re-label the vertical "Architecture-only — implementations on roadmap," matching the spec language already in the file headers ("permanently black-boxed"); or (b) actually implement at minimum a multi-qubit state vector simulator with universal gate set + measurement + a few of the named algorithms (VQE, QAOA, Grover) before continuing to advertise 224 jewels. The current state — `hasCode: true` on metadata-only records — is provably false at the field level and should be the first thing fixed. Note: this critique applies specifically to QUANTUM™. Other verticals may have higher implementation density (Cyber, Brain, Defense show real code per F-106/F-109/F-113); this finding does NOT generalize automatically.',
    recordedAt: '2026-04-18',
  },
  {
    id: 'F-115',
    title: 'ROBOTICS™ Crown Jewels — 160 entries marked `hasCode: true` are metadata-only; only ONE 156-LOC layer file backs 4 of the 16 advertised primitives with intro-level math',
    severity: 'THEATER',
    source: {
      document: 'src/crownjewels/robotics-vertical-registry.ts + src/crownjewels/a-tier/robotics-atier.ts + src/lib/factory/verticals/robotics.ts',
      quote: '160 Robotics Crown Jewels (80 S-tier + 80 A-tier) all hardcode `hasCode: true`. Marketing surface: robotics.cmpsbl.com, RoboticsHome.tsx, "ai-robotics-lab" hero asset. Factory file declares 16 robotics primitives (SERVO, KINETIC, LIDAR, FABRICATOR, FLUX, VECTOR, TENSOR, CALIBER, GRIPPER, SWARM, ENVIRON, MARSHAL, DISPATCH, WELDER, etc.) with capability strings.',
    },
    evidence: {
      reality:
        'Live audit 2026-04-18. (1) ENTRY COUNT — `grep -c "^  cj("` returns 80 + 80 = 160 records, all `hasCode: true`. (2) ACTUAL EXECUTABLE ROBOTICS CODE — one file: src/lib/export/layers/inventory/robotics-control-suite.layer.ts (156 LOC, dual TS+Python). Implements: cmpsbl_rcs_fabricator_cost (Euclidean path-length sum, ~10 LOC), cmpsbl_rcs_servo_new/step (textbook PID controller, ~15 LOC), cmpsbl_rcs_tensor_dot/norm/scale (3-vector ops, ~10 LOC), cmpsbl_rcs_lidar_min/safe (array min + threshold check, ~10 LOC). Covers 4 of 16 primitives (FABRICATOR, SERVO, TENSOR, LIDAR) at intro-textbook level. (3) MISSING — no inverse kinematics, no forward kinematics, no RRT/A*/PRM path planning, no SLAM, no occupancy grids, no trajectory optimization, no swarm coordination algorithms, no gripper force control, no calibration solvers, no ROS bridge, no MoveIt integration, no Gazebo/Webots adapter. (4) FACTORY VERTICAL — robotics.ts (451 LOC) is capability-string arrays only, no executors. (5) NO ROBOTICS SDKs in package.json (no rosnodejs, no roslibjs, no @foxglove/*). (6) Training corpus (round-10-robotics-embedded) contains ArduCopter.cpp + AP_HAL.h — third-party Ardupilot source, not CMPSBL implementation. The entire Robotics vertical operationally = path-length sum + PID + dot product + array min.',
      method: 'find src -iname "*robotic*"; grep -c "^  cj("; wc -l on robotics-control-suite.layer.ts + robotics.ts; grep for inverseKinematic/forwardKinematic/pathPlan/RRT/SLAM across src/; package.json scan for ROS/robotics SDKs.',
    },
    verdict:
      'Same architectural pattern as F-114 (Quantum) but ~30% better real-code coverage: Robotics has 4 of 16 primitives implemented at intro level vs Quantum\'s 4 of 16 at intro level — both verticals share the "1 layer file backs 16-primitive marketing claim" structural lie. PID + path-length-sum is real and functional, but it does not constitute "160 Crown Jewels of robotics IP." Honest fix: flip `hasCode: false` on the 156 unimplemented entries, keep `true` only on the 4 covered primitives (FABRICATOR cost, SERVO PID, TENSOR vec3, LIDAR safety), and re-label the rest "Architecture-only — implementations on roadmap." This finding plus F-114 establishes a PATTERN: the cj()-factory + single-layer-file structure likely recurs across most verticals (Cyber/Brain/Defense excepted per F-106/F-109/F-113). Recommend a sweep audit of all remaining vertical registries (Fintech, Health, Education, Gaming, Media, Mana, Ultimate) to confirm or refute pattern-wide.',
    recordedAt: '2026-04-18',
  },
  {
    id: 'F-116',
    title: 'Front-facing website (cmpsbl.com homepage) — 25-claim audit: 11 FACT, 8 PARTIAL, 4 THEATER, 2 FICTION',
    severity: 'PARTIAL',
    source: {
      document: 'https://www.cmpsbl.com (homepage, fetched 2026-04-18)',
      quote: 'Full-page audit of all marketing claims surfaced above the fold and through CTA cards. Each claim cross-checked against codebase, NPM registry, external services (Zenodo, ORCID), and database state.',
    },
    evidence: {
      reality:
        'CLAIM-BY-CLAIM VERDICT (live audit 2026-04-18). ✅ C1 "20 Exclusive Layers" — FACT+. Actual count: 25 layer files in src/lib/export/layers/inventory/. ✅ C2 "9 verified language targets" (TS, JS, Py, Rust, Go, Java, Kotlin, C#, Swift) — FACT. ✅ C3 "11 NPM packages" — FACT (under-claim). Actual: 13 packages live (cli@4.2.0, sdk@4.1.0, runtime@4.3.0, mana@3.1.0, shield@3.1.0, bridge@3.1.0, discovery@3.1.0, failsafe@5.1.0, intent@3.1.0, mesh@3.1.0, react@3.1.0, test-harness@3.1.0, types@3.1.0). Codebase hardcodes "11" in 5 files (FreeValueProposition.tsx:11, LiveStatsBar.tsx:30, InvestorDocLibrary.tsx:104/352/622). ✅ C4 "25 languages on parity roadmap" — FACT. ✅ C5 "40 Primitives" — FACT (canon). ⚠️ C6 "8hr autonomous cycles" — PARTIAL. No interval constant in src/lib/memorystream/. Cycle exists per F-103/F-113 but cadence not code-enforced. ✅ C7 Patent numbers (64/029,678 + 64/031,637) — FACT. Referenced in 145 source files. ⚠️ C8 NPM org page link — PARTIAL. Direct package GETs return 200 but `https://www.npmjs.com/org/cmpsbl` returns HTTP 403. "View All Packages on NPM" CTA broken for visitors. ✅ C9 cli + sdk on NPM — FACT. ✅ C10 Zenodo v13.5 — FACT (200). ✅ C11 ORCID 0009-0001-4237-1243 — FACT (200). ✅ C12 Lex Governance — FACT (lex.ts + lex-extended.ts present). ⚠️ C13 "Silently attached Layer 2 to lodash" demo — PARTIAL. ManaProof.tsx exists but the displayed function calls (lodash.get → "42", lodash.debounce → "🛑 MANA says: Simon says no") are HARDCODED RESULT STRINGS in a static array, not live execution. Real packages exist but the on-page demo is a styled mockup. ✅ C14 "SHA-256 verification" — FACT (crypto.subtle.digest in 4 files). ✅ C15 "Ascension v2 in production" — FACT (AscensionV2.tsx routed). ⚠️ C16 "Sealed Mini-Runtime™" — PARTIAL. String only appears in test files (10-language-smoke.test.ts, e2e-ascension-smoke.test.ts), not production exporter code. Either renamed or aspirational. ❌ C17 "90+ languages" / "Ship in 90+ languages" — THEATER. src/lib/export/emitters/ does not exist. Polyglot system supports the 9 verified + 25 roadmap = 34 max planned. The 90+ figure is unsupported by any artifact. THIS IS THE LARGEST WEBSITE LIE. ⚠️ C18 "143+ Primitives (Architect ULTIMATE™)" — PARTIAL. licensing-products.ts hardcodes "All 143+ primitives" but canonical doctrine is 40 Primitives. Reconciles only by counting per-vertical primitives (40 core + 12 verticals × ~8 ≈ 136). Honest framing: "40 core + 100+ vertical primitives." ⚠️ C19 Tiers ($0 / $29 / $49 / $79 / $999+) — PARTIAL. pricing-v7.ts shows $49 / $129 / $149 / $249 / $299 — homepage and product price file do not match. ✅ C20 "Free Forever / OG Prime" — FACT. ❌ C21 Testimonials (Daniel Okafor/Relay Systems, Priya Nair/Arcline Analytics, Marcus Chen/Lattice Dynamics) — FICTION. All three are hardcoded strings in src/components/home/SocialProof.tsx with no source documents, no verifiable companies, no LinkedIn corroboration. Project doctrine bans "our team" framing because Kenneth is solo founder — these fabricated customer quotes contradict that identity. ⚠️ C22 "16 verified case studies" — PARTIAL. Real downloadable artifact pairs exist (huggingface-tokenizers, a-star) with original + ascended zips. Count of 16 needs verification; 2 visible in snippet. ✅ C23 "Zero AI in Output" / "Zero external AI calls" — FACT (project-knowledge confirmed). ⚠️ C24 "Mana Symbiosis ships with every artifact" — PARTIAL. Mana runtime IS real, but auto-injection into every Ascension export is unverified — Mana attachment is described as Step 2 (optional) per F-103. ❌ C25 "Where machines learn how to dream" + DREAM.resilience/trust/clarity/conviction in homepage code animation — THEATER. DREAM is real (no AI inside, algorithmic) but the on-page labels are decorative SVG strings in CodeWrapAnimation.tsx, not live module exports. Substance real; demo theater. SUMMARY: 11 FACT (44%), 8 PARTIAL (32%), 4 THEATER (16%), 2 FICTION (8%).',
      method: 'Fetched homepage via lov-fetch-website; cross-checked each surfaced claim against (a) src/ greps, (b) live NPM registry GETs for each @cmpsbl package, (c) HEAD requests on Zenodo + ORCID, (d) inspection of SocialProof.tsx + ManaProof.tsx + licensing-products.ts + pricing-v7.ts.',
    },
    verdict:
