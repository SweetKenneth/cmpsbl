 ...
 * Wide layout, spacious cards, hover effects, multi-column grids
 */
...
const FILTER_CONFIG = [
  { key: "all" as const, label: "All Products", icon: ShoppingBag, count: ALL_STORE_ITEMS.length },
  { key: "agents" as const, label: "Runtime Agents", icon: Users, count: STORE_AGENTS.length },
      ...
      <main className="min-h-screen pt-32 pb-24">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
            ...
            >
              <TabsList className="h-14 p-1.5 bg-card/60 backdrop-blur-md border border-border/40 rounded-2xl gap-1 shadow-lg shadow-background/50">
                {TAB_CONFIG.map(({ value, label, icon: Icon }) => (
                  <TabsTrigger
                    key={value}
                    value={value}
                    className={cn(
                      "gap-2 px-7 h-10 rounded-xl text-sm font-bold transition-all min-w-[100px]",
            ...
            <TabsContent value="store" className="mt-0">
                ...
                {TIERS.map((t, i) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={cn(
                        "px-3.5 py-1.5 rounded-full border text-[11px] font-black tracking-wider",
              ...
              <div className="flex items-center justify-center gap-2 mb-10">
                {FILTER_CONFIG.map(({ key, label, icon: Icon, count }) => (
                  <Button
                    ...
                    size="sm"
                    className={cn(
                      "gap-1.5 text-xs font-bold min-h-[44px] px-5 rounded-xl transition-all duration-300",
                      filter === key
              ...
              {/* Sealed Runtime Features — 3 column */}
              <motion.section
                    ...
                    <div className="grid grid-cols-3 gap-4">
                      {SEALED_FEATURES.map((feat, i) => (
                        <motion.div
                          ...
                          className={cn(
                            "group rounded-2xl border border-border/30 bg-card/40 backdrop-blur-sm p-5",
                        ...
                        >
                          <div className="flex items-center gap-3 mb-2.5">
                            ...
                            <h4 className="text-sm font-black text-foreground tracking-tight">{feat.label}</h4>
                          </div>
                          <p className="text-xs text-muted-foreground/70 leading-relaxed pl-11">{feat.desc}</p>
                        </motion.div>
                      ))}
                    </div>
                  </div>
