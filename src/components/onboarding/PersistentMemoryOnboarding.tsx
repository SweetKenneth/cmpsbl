...
export function PersistentMemoryOnboarding() {
  const [open, setOpen] = useState(false);
  const [step, setStep] = useState(0);
  const [direction, setDirection] = useState<'next' | 'prev'>('next');
  const { trackOpened, trackStep, trackCompleted, trackSkipped } = useOnboardingTracking('persistent-memory');
      ...
      <div className="relative w-full max-w-md cmpsbl-welcome-modal-enter">
        <div className="relative bg-background/95 border border-border/30 rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="relative flex items-center justify-between px-5 pt-4">
            <span className="text-[10px] font-mono uppercase tracking-[0.2em] text-muted-foreground/50">PERSISTENT MEMORY</span>
