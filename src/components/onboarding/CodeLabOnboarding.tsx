...
const STEPS: OnboardingStep[] = [
  ...
  },
  {
    ...
    tag: 'Forge',
    title: 'Signal Forge',
    body: 'Hit the Forge button and the system discovers a new blueprint by mapping capabilities across 40 primitives. Each result is scored, validated, and ready to build on.',
    accent: 'neon-cyan',
  ...
  },
  {
    ...
    body: 'Write resolver calls and execute them in real time. Inspect responses, review telemetry, and iterate — your history persists across sessions.',
    accent: 'neon-green',
  ...
  },
  {
    ...
    body: 'Everything below is interactive. Here\'s how to begin:',
    accent: 'neon-amber',
        ...
        <div className="relative bg-background/95 border border-border/30 rounded-2xl shadow-2xl overflow-hidden">
          ...
          <div className="relative flex items-center justify-between px-6 pb-5 pt-1 border-t border-border/10">
            ...
            <Button size="sm" onClick={next} className="text-xs h-9 gap-1.5 px-5 cmpsbl-welcome-cta-glow">
              {isLastStep ? 'Start Building →' : (<>Next <ArrowRight className="w-3.5 h-3.5" /></>)}
            </Button>
          </div>
        </div>
      </div>
