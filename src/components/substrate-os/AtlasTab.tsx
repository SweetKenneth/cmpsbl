...
import { 
  ...
  Clock, Activity, Brain, TestTube, BookOpen
} from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold">Atlas Control Plane</h2>
            <p className="text-xs text-muted-foreground font-mono">v7.0.0 • single-source-of-truth interface</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {capabilities.map(cap => (
                    <div key={cap.key} className="flex items-center justify-between">
                      <span className="text-xs font-mono">{cap.key}</span>
                      <Switch
                        checked={cap.enabled}
          ...
          <Card className="border-purple-500/20 bg-muted/5">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium">SEBA</p>
                  <p className="text-[10px] text-muted-foreground">{sebaStatus?.mode || 'off'} • {sebaStatus?.phase || 'idle'}</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs font-medium">Autoblog</p>
                  <p className="text-[10px] text-muted-foreground">{autoblogStatus?.mode || 'off'} • {autoblogStatus?.queue_length || 0} queued</p>
                </div>
