...
const AFFINITY_PAIRS = [
  ...
  { a: 'VISION', b: 'CORTEX', score: 0.88, label: 'Perception memory chain' },
...
function generateHubMessages(): HubMessage[] {
  const base: HubMessage[] = [
    ...
    { id: 'h6', source: 'CORTEX', type: 'proposal', summary: 'Memory efficiency can be improved 12% by reordering ENCODE → NEXUS flow', detail: 'Analysis of 50K recent memory executions shows that routing ENCODE output through NEXUS before FORGE reduces redundant token generation by 12%. Requires memory configuration update and 10-minute warm-up period.', urgency: 'medium', age: '8m', status: 'pending' },
    ...
    { id: 'h10', source: 'ATLAS', type: 'status', summary: 'Governance audit complete — all 40 primitives reporting healthy', detail: 'Scheduled governance sweep completed. All 40 substrate primitives passed health checks. 2 flagged for minor configuration drift (auto-corrected). Next audit scheduled in 6 hours.', urgency: 'low', age: '45m', status: 'acknowledged' },
    { id: 'h11', source: 'NERVE', type: 'alert', summary: 'Communication latency spike detected between BRAIN and MEMORY organs', detail: 'Inter-primitive messaging latency increased from 2ms to 45ms. Possible congestion in the internal bus. NERVE is rerouting through backup channels. If latency persists beyond 10 minutes, recommend mesh reboot.', urgency: 'high', age: '3m', status: 'pending' },
    ...
    { id: 'h13', source: 'FORGE', type: 'status', summary: 'Cognitive minting queue processed — 3 new Convex Core™ artifacts deployed', detail: 'Three cognitives passed validation and were sealed: DataAnalyst-v2, SecurityAuditor-v1, ContentOptimizer-v3. All include embedded skill weights and capability manifests.', urgency: 'low', age: '35m', status: 'acknowledged' },
    ...
    { id: 'h21', source: 'RIPPLE', type: 'status', summary: 'Message bus throughput nominal — 1,240 events/min processed', detail: 'All 40 primitive subscriptions active. Average message latency: 3.2ms. No dropped messages in the last 6 hours. Queue depth: 0 (healthy).', urgency: 'low', age: '5m', status: 'acknowledged' },
    ...
    { id: 'h24', source: 'VISION', type: 'proposal', summary: 'Performance regression detected in blog rendering memory chain', detail: 'Page load time for /blog increased from 1.2s to 2.8s over the past week. Root cause: unoptimized image memory chain and excessive client-side re-renders. Recommend lazy loading and image CDN integration.', urgency: 'medium', age: '30m', status: 'pending' },
    { id: 'h25', source: 'CORTEX', type: 'status', summary: 'Memory orchestration health check passed — all chains operational', detail: '14 active memory chains verified. No stale locks detected. Average chain execution time: 340ms. Longest chain: SCAN→ANALYZE→REPORT at 890ms (within SLA).', urgency: 'low', age: '15m', status: 'acknowledged' },
    ...
    { id: 'h38', source: 'NERVE', type: 'status', summary: 'Inter-primitive communication matrix — all 40 primitives connected', detail: 'Full mesh connectivity confirmed. Average cross-primitive latency: 4.1ms. No partition events detected. Heartbeat failure count: 0 in last 24h.', urgency: 'low', age: '10m', status: 'acknowledged' },
    ...
    { id: 'h42', source: 'GOVERNANCE', type: 'status', summary: 'Policy compliance report — 97.3% compliance across all primitives', detail: '36 of 37 auditable policies passing. 1 warning: ENCODE code generation policy needs updated test coverage (last tested 14 days ago). No critical violations.', urgency: 'low', age: '1h', status: 'acknowledged' },
    ...
    { id: 'h47', source: 'CORTEX', type: 'alert', summary: 'Memory chain timeout on SCAN→DEEP-ANALYZE — exceeded 30s SLA', detail: 'Deep analysis memory took 47s on a complex multi-file scan. Bottleneck identified in the pattern matching phase. CORTEX is implementing chunked processing to prevent future timeouts.', urgency: 'medium', age: '12m', status: 'pending' },
