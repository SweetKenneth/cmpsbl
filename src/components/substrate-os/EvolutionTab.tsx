 ...
 * v10.5.4 ARCHITECT Epoch — Living Substrate Orchestration
 * 
...
import { 
  Dna, Play, Pause, History, Settings, Zap, 
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-fuchsia-500/30 to-purple-500/20 border border-fuchsia-500/40 flex items-center justify-center">
            <Dna className="w-5 h-5 text-fuchsia-400" />
          </div>
          <div>
            <h2 className="text-lg font-semibold text-foreground">Self-Evolution</h2>
            <p className="text-xs text-muted-foreground">Living Substrate Orchestrator v{status?.version || '1.0.0'}</p>
          </div>
        </div>
          ...
          <CardContent className="pt-4">
            {statusLoading ? (
              ...
              <div className="text-center">
                <p className="text-3xl font-bold text-fuchsia-400">{status?.cycles_total || 0}</p>
        ...
        <Card className="bg-white/5 border-white/10">
          <CardContent className="pt-4">
            {statusLoading ? (
              ...
              <div className="text-center">
                <p className="text-3xl font-bold text-cyan-400">{status?.cycles_today || 0}</p>
                ...
                <div className="space-y-2">
                  {(['scan', 'generate', 'sandbox', 'apply', 'learn'] as const).map((phase) => (
                    <div key={phase} className="flex items-center gap-3">
                      <div className={cn("w-6 h-6 rounded-full flex items-center justify-center bg-white/5", 
                        phaseColors[currentCycle?.phases?.[phase]?.status as keyof typeof phaseColors || 'pending']
                      )}>
                        {phaseIcons[phase]}
                      </div>
            ...
            <div className="flex gap-2">
              <AlertDialog>
                <AlertDialogTrigger asChild>
                  <Button 
                    className="flex-1 gap-2 bg-fuchsia-500/20 hover:bg-fuchsia-500/30 text-fuchsia-400 border border-fuchsia-500/40"
      ...
      <Card className="bg-white/5 border-white/10">
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
            {[
              { phase: 'SCAN', icon: Zap, color: 'cyan', desc: 'Modernizer detects improvements' },
              { phase: 'GENERATE', icon: Code, color: 'fuchsia', desc: 'Substrate Coder writes code' },
              { phase: 'SANDBOX', icon: FlaskConical, color: 'amber', desc: 'E2B tests in isolation' },
              { phase: 'APPLY', icon: Rocket, color: 'emerald', desc: 'Deploy to shadow/production' },
              { phase: 'LEARN', icon: Brain, color: 'purple', desc: 'Brain records patterns' },
            ].map((item, idx) => (
