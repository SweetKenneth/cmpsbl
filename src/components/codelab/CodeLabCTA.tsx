...
import { Code, ArrowRight, Brain, Shield, Moon, Sparkles } from "lucide-react";
        ...
        <div className="relative p-6 sm:p-8 md:p-12 lg:p-16 rounded-3xl overflow-hidden border border-primary/20">
          ...
          <div className="relative flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12">
            ...
            <div className="flex lg:flex-col gap-4 lg:gap-6 shrink-0">
              {[
                { icon: Brain, label: "Memory", color: "text-cyan-500 bg-cyan-500/10" },
                { icon: Shield, label: "Defense", color: "text-amber-500 bg-amber-500/10" },
                { icon: Moon, label: "Dream", color: "text-violet-500 bg-violet-500/10" },
              ].map((item) => (
                <motion.div
                  key={item.label}
                  whileHover={{ scale: 1.05 }}
                  className={`w-14 h-14 sm:w-16 sm:h-16 rounded-xl flex flex-col items-center justify-center gap-1 ${item.color}`}
                >
