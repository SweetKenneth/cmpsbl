 ...
 * Uses pure CSS transform for 3D flip to ensure backfaceVisibility works.
 */
          ...
          <div className="relative z-10 px-4 pb-4 -mt-12 shrink-0">
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/30">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-mono tabular-nums text-muted-foreground/60">{agent.powers.length} POWERS</span>
              </div>
              <div className="flex gap-1">
                {agent.fusedFrom.map((name) => (
                  <span key={name} className="text-[7px] font-mono font-bold px-1 py-0.5 rounded bg-foreground/5 text-muted-foreground/40 border border-border/20">{name}</span>
                ))}
              </div>
            </div>
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className={cn("px-2 py-0.5 rounded-full text-[8px] font-black border", tier.bg, tier.text, tier.border)}>{tier.label}</div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1">
                {agent.clmGoals.map((goal) => (
                  <span key={goal} className="text-[9px] font-medium px-2 py-0.5 rounded-full bg-primary/8 text-muted-foreground/70 border border-primary/10">{goal}</span>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="p-4 border-t border-border/30 space-y-2 shrink-0">
            ...
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="flex-1 gap-1.5 text-[10px] font-bold"
                onClick={(e) => { e.stopPropagation(); onChat(); }}>
                <MessageSquare className="w-3 h-3" /> DECODE Channel
              </Button>
              <Button variant="ghost" size="sm" className="text-[10px] font-bold gap-1"
