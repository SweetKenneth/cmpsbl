...
function DiscoveryCard({ discovery, index }: { discovery: Discovery; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="bg-card/30 border border-border/20 rounded-lg p-3 sm:p-4 backdrop-blur-sm hover:border-border/30 hover:bg-card/50 transition-all duration-300"
    >
      <div className="flex items-start justify-between gap-2 sm:gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 sm:gap-2 mb-1">
            <span className={`text-[9px] sm:text-[10px] font-mono px-1.5 sm:px-2 py-0.5 rounded border ${tierStyle} uppercase tracking-wider`}>
              {discovery.tier === 'cmpsbl-only' ? 'APEX' : discovery.tier}
            </span>
            <span className="text-[9px] sm:text-[10px] text-muted-foreground/50 uppercase tracking-wider truncate">
              {discovery.category}
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-1 mt-1.5 sm:mt-2">
            {discovery.module_chain.slice(0, 4).map(m => (
              <span key={m} className="text-[8px] sm:text-[9px] font-mono px-1 sm:px-1.5 py-0.5 rounded bg-muted/30 text-muted-foreground">
                {m}
              </span>
            ))}
            {discovery.module_chain.length > 4 && (
              <span className="text-[8px] sm:text-[9px] font-mono text-muted-foreground/40">
                +{discovery.module_chain.length - 4}
              </span>
            )}
          </div>
        ...
        <div className="text-right shrink-0">
          ...
          <div className="text-[8px] sm:text-[9px] text-muted-foreground uppercase tracking-wider">CJPI</div>
        </div>
      </div>
    </motion.div>
  );
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center justify-between mb-10 sm:mb-12">
          <div>
            ...
            <p className="text-muted-foreground/60 text-xs sm:text-sm mt-1">
        ...
        <div className="flex justify-center gap-1.5 mt-6 sm:mt-8">
          ...
          {totalSets > 20 && (
            <span className="text-[9px] font-mono text-muted-foreground/40 ml-1 self-center">
              +{totalSets - 20}
            </span>
          )}
        </div>
