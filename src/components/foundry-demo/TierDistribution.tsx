  ...
  return (
    <section className="py-16 sm:py-20 md:py-40 px-5 sm:px-6 relative">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-center text-muted-foreground/50 text-xs sm:text-xs font-mono mb-10 sm:mb-16 px-2">
          {total.toLocaleString()} programs · 50.6% Apex tier · Zero below CJPI 80
        </p>

        <div className="h-12 sm:h-14 rounded-lg overflow-hidden flex mb-6 sm:mb-8 border border-border/10">
          {TIERS.map((t) => (
            <div
              key={t.tier}
            ...
            >
              <span className="text-xs sm:text-xs font-mono font-bold text-black/70 group-hover:opacity-100 opacity-70 transition-opacity">
                {t.percentage}
              </span>
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          {TIERS.map((t, i) => (
            ...
            >
              <div className="w-3.5 h-3.5 rounded-sm mt-0.5 shrink-0" style={{ backgroundColor: t.color }} />
              <div>
                <div className="font-mono text-sm sm:text-sm font-bold text-foreground">
                  {t.label} <span className="text-muted-foreground font-normal">· {t.count}</span>
                </div>
                <div className="text-xs sm:text-xs text-muted-foreground/60 mt-0.5">{t.description}</div>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        <div className="mt-10 sm:mt-12 text-center">
          <div className="inline-flex items-center gap-2 bg-card/50 border border-border/20 rounded-lg px-5 sm:px-5 py-2.5 sm:py-2.5">
