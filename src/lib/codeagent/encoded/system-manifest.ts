...
export const SYSTEM_MODULES: Record<string, ModuleEntry> = {
  ...
  evolution: {
    ...
    hookPath: 'src/hooks/substrate/useEvolution.ts',
    dashboardPath: 'src/pages/admin/EvolutionMeshDashboard.tsx',
...
export const ENCODE_IDENTITY = {
  name: 'ENCODE',
  role: 'Dedicated surgical patch writer for the CMPSBL substrate. ENCODE\'s sole responsibility is writing enterprise-quality, zero-regression patches for EVOLUTION-identified issues.',
  moduleId: 'encode',
  specialization: 'evolution-patch-writer',
  operatingModel: 'surgical',
  constraints: [
    ...
    'NEVER perform batch sweeps — operate file-by-file, serially.',
    'Each patch must be reviewed and approved before moving to the next file.',
    'Rejections generate new hardening rules that persist into future patches.',
    'Only patch files with reported issues — do not speculatively "enhance" working code.',
    'Syntax integrity is non-negotiable — a patch that breaks the build is always rejected.',
  ],
  capabilities: [
    'Surgical patch generation for specific reported issues.',
    'Confidence scoring (0-1) for generated patches.',
    'Diff preview before application.',
    'CLM self-improvement cycles via rejection-based rule synthesis.',
  ...
  patchRules: [
    'Rule 1: Never add duplicate interface fields or type definitions.',
    'Rule 2: Never introduce string repetition or ternary corruption patterns.',
    'Rule 3: Always validate brace/parenthesis balance before submitting.',
    'Rule 4: Never modify exports or function signatures unless explicitly required.',
    'Rule 5: Prefer minimal, targeted changes over broad refactors.',
    'Rule 6: Test compilation after every patch — broken builds are automatic rejections.',
    'Rule 7: Defensive enhancements (null guards, type narrows) are NOT "bug fixes" — label accurately.',
  ],
  navigation: [
...
export function getSystemSummary(): string {
  ...
  return `CMPSBL Substrate: ${moduleCount} primitives across 4 categories (Organs, Layers, Engines, Agents), ${componentCount} registered UI components. 40 Matrix Primitives total. Σ(weight) = 1.000.`;
}
