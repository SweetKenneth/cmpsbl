...
function inferModules(summary: string): string[] {
  const moduleKeywords: Record<string, string[]> = {
    ...
    DECODE: ['interpret', 'parse', 'classify', 'intent', 'understand'],
    BRAIN: ['memory', 'recall', 'remember', 'learn', 'knowledge'],
    DEFENSE: ['security', 'threat', 'guard', 'protect', 'rate limit'],
    NEXUS: ['route', 'api', 'provider', 'model', 'ai'],
    EVOLUTION: ['evolve', 'mutate', 'improve', 'upgrade', 'modernize'],
    SYSTEM: ['config', 'setting', 'system', 'boot', 'health'],
    VISION: ['scan', 'monitor', 'observe', 'watch', 'detect'],
    CORTEX: ['orchestrate', 'coordinate', 'pipeline', 'workflow'],
    MEMORY: ['store', 'persist', 'cache', 'retrieve', 'gc'],
  };
