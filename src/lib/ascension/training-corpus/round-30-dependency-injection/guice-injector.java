# Content from https://raw.githubusercontent.com/google/guice/master/core/src/com/google/inject/internal/InjectorImpl.java
...
final class InjectorImpl implements Injector, Lookups {
 public static final TypeLiteral STRING\_TYPE = TypeLiteral.get(String.class);
 ...
 try {
 var result = getBindingOrThrow(key, errors, JitLimitation.EXISTING\_JIT);
 ...
 ConstantFactory.create(membersInjector, SourceProvider.UNKNOWN\_SOURCE);

 return new InstanceBindingImpl>(
 this,
 key,
 SourceProvider.UNKNOWN\_SOURCE,
 factory,
 ImmutableSet.of(),
 membersInjector);
 }
 ...
 BindingImpl delegate = getBindingOrThrow(providedKey, errors, JitLimitation.NO\_JIT);
 return new SyntheticProviderBindingImpl(this, key, delegate);
 }

 /\\*\\* A framework-created JIT {@code Provider} binding. \*/
 ...
 @Override
 public Provider\> makeProvider(InjectorImpl injector, Dependency dependency) {
 ...
 Key stringKey = key.ofType(STRING\_TYPE);
 ...
 /\\*\\* For multibinding bindings that delegate to each other. \*/
  void initializeBindingIfDelayed(Binding binding, Errors errors) throws ErrorsException {
 ...
 private  void initializeJitBinding(BindingImpl binding, Errors errors)
 throws ErrorsException {
 ...
 Errors errors,
 boolean jitBinding,
 Consumer creationListenerCallback)
 throws ErrorsException {
 ...
 InternalFactory\> factory =
 ConstantFactory.create(value, SourceProvider.UNKNOWN\_SOURCE);
 return new InstanceBindingImpl>(
 this,
 key,
 SourceProvider.UNKNOWN\_SOURCE,
 ...
 if (parent != null) {
 if (jitType == JitLimitation.NEW\_OR\_EXISTING\_JIT
 && jitDisabled
 ...
 new Errors(),
 jitDisabled,
 parent.options.jitDisabled ? JitLimitation.NO\_JIT : jitType);
...
4. The constructor of the raw type. Only for unannotated keys.
 ...
 \*/
 private  BindingImpl createJustInTimeBinding(
 Key key, Errors errors, boolean jitDisabled, JitLimitation jitType)
 throws ErrorsException {
 int numErrorsBefore = errors.size();
 ...
 // createSyntheticProviderBinding() will return BindingImpl>.
 @SuppressWarnings("unchecked")
 BindingImpl binding = (BindingImpl) createSyntheticProviderBinding((Key) key, errors);
 return binding;
 }
 ...
 if (!isTypeLiteral(key) && jitDisabled && jitType != JitLimitation.NEW\_OR\_EXISTING\_JIT) {
 throw errors.jitDisabled(key).toException();
 }
 ...
 return getBindingOrThrow(key.withoutAttributes(), ignored, JitLimitation.NO\_JIT);
 } catch (ErrorsException ignored) {
 ...
 Key key, Errors errors, JitLimitation jitType) throws ErrorsException {
 ...
 final Dependency dependency, final Errors errors) throws ErrorsException {
 BindingImpl binding =
 getBindingOrThrow(dependency.getKey(), errors, JitLimitation.NO\_JIT);
 ...
 Provider result = getBindingOrThrow(key, errors, JitLimitation.NO\_JIT).getProvider();
 ...
 // that getProvider can leverage the cache inside BindingImpl
  Provider getProviderForBindingImpl(Key key) {
 Errors errors = new Errors(key);
 try {
 ...
 // Used by LookupProcessor to satisfy delegates.
  Provider getProviderOrThrow(Dependency dependency, Errors errors)
 throws ErrorsException {
 var key = dependency.getKey();
 BindingImpl binding = getBindingOrThrow(key, errors, JitLimitation.NO\_JIT);
 @SuppressWarnings("unchecked") // safe because Providers are covariant.
 Provider provider =
 (Provider) binding.getInternalFactory().makeProvider(this, dependency);
 return provider;
 }
...
All callers of this are responsible for calling {@link InternalContext#close()}. Typical
