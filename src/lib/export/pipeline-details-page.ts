...
const MODULE_CAPABILITIES: Record<string, ModuleCapability> = {
  BRAIN: {
    ...
    behaviourDetail: 'BRAIN Organ operates as the memory chain\'s central decision-maker. When invoked, it decomposes a problem into sub-goals, evaluates each against available evidence, and converges on a recommended action. It maintains a reasoning trace that downstream primitives can audit or override.',
  },
  CORTEX: {
    name: 'CORTEX Engine',
    role: 'Pattern Analysis & Recognition Engine',
    ...
    behaviourDetail: 'CORTEX Engine scans incoming data for known pattern signatures and novel formations. When paired with BRAIN Organ, it provides the evidence layer that reasoning depends on. Standalone, it functions as a high-throughput classifier.',
  },
  DREAM: {
    name: 'DREAM Engine',
    ...
    behaviourDetail: 'DREAM Engine doesn\'t operate on real data — it imagines possibilities. It takes the current state, mutates variables within defined boundaries, and evaluates simulated outcomes. This makes it invaluable for strategy exploration without production risk.',
  },
  EVOLUTION: {
    name: 'EVOLUTION Layer',
    role: 'Adaptive Optimization Layer',
    ...
    behaviourDetail: 'EVOLUTION Layer runs a population of candidate solutions through selection, crossover, and mutation cycles. After convergence, it outputs the fittest configurations. When combined with DREAM Engine, it can evolve strategies in simulated environments before applying them to production.',
  },
  VISION: {
    name: 'VISION Agent',
    role: 'Structural & Visual Pattern Agent',
    ...
    behaviourDetail: 'VISION Agent treats your system as a graph and applies structural analysis to identify bottlenecks, circular dependencies, and over-coupled components. It provides actionable refactoring paths with estimated impact scores.',
  },
  DEFENSE: {
    name: 'DEFENSE Layer',
    role: 'Security Boundary Enforcement Layer',
    ...
    behaviourDetail: 'DEFENSE Layer acts as a security gate within the memory chain. Every operation passes through its policy engine before execution. It maintains a threat model that adapts based on observed attack patterns and can quarantine suspicious operations without blocking the entire chain.',
  },
  GOVERNANCE: {
    name: 'GOVERNANCE Layer',
    role: 'Policy Compliance & Oversight Layer',
    whatItDoes: 'Ensures all memory chain operations comply with defined policies, regulations, and operational constraints. It audits decisions and flags non-compliant outputs before they reach production.',
    ...
    behaviourDetail: 'GOVERNANCE Layer wraps the memory chain in a compliance layer. It intercepts outputs, checks them against rule sets, and either approves, flags for review, or blocks non-compliant results. It generates immutable audit trails for regulatory evidence.',
  },
  MEMORY: {
    name: 'MEMORY Organ',
    role: 'Persistent State & Recall Organ',
    whatItDoes: 'Stores, indexes, and retrieves information across memory chain executions. It provides long-term recall that allows the chain to learn from its own history and avoid repeating mistakes.',
    ...
    behaviourDetail: 'MEMORY Organ gives the memory chain a persistent identity. Without it, each execution is stateless. With it, the chain can recall previous inputs, results, and errors — enabling progressive improvement and contextual awareness across sessions.',
  },
  NEXUS: {
    name: 'NEXUS Organ',
    role: 'Signal Routing & Coordination Organ',
    whatItDoes: 'Routes data between memory chain stages, external APIs, and downstream consumers. It handles load balancing, retry logic, and protocol translation between heterogeneous systems.',
    ...
    behaviourDetail: 'NEXUS Organ is the memory chain\'s nervous system. It ensures that data reaches the right primitive at the right time, handles failures gracefully with retry and fallback strategies, and provides delivery guarantees across unreliable networks.',
  },
  RIPPLE: {
    name: 'RIPPLE Organ',
    role: 'State Change Propagation Organ',
    whatItDoes: 'When one part of the system changes, RIPPLE Organ propagates that change to all dependent components. It manages cascading updates, ensures consistency, and prevents stale state.',
    ...
    behaviourDetail: 'RIPPLE Organ ensures that a change in one primitive doesn\'t leave the rest of the system out of sync. It traverses the dependency graph, updates affected components in topological order, and reports any propagation failures for manual resolution.',
  },
  SYSTEM: {
    name: 'SYSTEM Organ',
    role: 'Core Orchestration & Lifecycle Organ',
    whatItDoes: 'Manages the memory chain\'s lifecycle — initialization, execution sequencing, health monitoring, graceful shutdown, and error recovery. It\'s the conductor that keeps all primitives working in concert.',
    inputsAccepted: 'Memory chain configuration, execution schedules, health thresholds, and recovery policies.',
    outputProduced: 'Execution status reports, health dashboards, lifecycle events, and recovery logs.',
    behaviourDetail: 'SYSTEM Organ is the memory chain\'s operating system. It boots primitives in dependency order, monitors their health during execution, handles failures according to recovery policies, and ensures clean shutdown with state preservation.',
  },
};
...
<div class="page">
  ...
  <footer class="colophon">
    <div class="colophon-text">
      ...
      Pipeline valuations are heuristic estimates based on technical scoring and may not reflect actual market value.<br>
