 ...
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 * Static verification that the Java emitter produces well-formed source for
 * every supported layer combination. Mirrors the Go/Rust harnesses.
 *
 * Runs in the browser without a JDK by performing structural / lexical checks:
 *
 *   1. emitJavaCmpsblFile renders without throwing for every selected subset
 ...
 *   5. Brace, paren, and bracket count balance (Java-aware comment/string skip)
 *
 * © CMPSBL® — All rights reserved.
 */

import { JAVA_LAYER_BODIES } from './java-layers';
import { emitJavaCmpsblFile, orderJavaLayersByPhase } from './java-chain-executor';

export interface JavaParityResult {
  passed: boolean;
  totalCases: number;
...
const ALL_JAVA_LAYER_IDS = Object.keys(JAVA_LAYER_BODIES);

function buildCases(): Array<{ name: string; ids: string[] }> {
  const cases: Array<{ name: string; ids: string[] }> = [
    { name: 'empty selection', ids: [] },
    { name: 'all 20 layers', ids: ALL_JAVA_LAYER_IDS },
  ];
  for (const id of ALL_JAVA_LAYER_IDS) cases.push({ name: `single: ${id}`, ids: [id] });
...
export function runJavaParityTests(): JavaParityResult {
  ...
  for (const c of cases) {
    let src: string;
    try {
      src = emitJavaCmpsblFile('com.cmpsbl.ascension', c.ids);
    } catch (err) {
    ...
    const checks: Array<string | null> = [
      ...
      assertContains(src, 'public final class Cmpsbl', 'top-level Cmpsbl class'),
      assertContains(src, 'public static Map<String, Object> cmpsblExecute(', 'phase-locked entry point'),
      assertContains(src, 'public static final class Layer1', 'Layer 1 declaration'),
    ...
    const ordered = orderJavaLayersByPhase(c.ids);
