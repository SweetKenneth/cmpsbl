...
registerNative('isolated-executor', 'rust', () => [
  ...
  '// Sandboxed wrapper for closures that may fail. Renamed to cmpsbl_isolate to',
  '// avoid colliding with the canonical pipeline executor cmpsbl_execute(name, JsonMap)',
  '// emitted by the polyglot template. The pipeline executor and the closure',
  '// isolator are two different surfaces — one names a capability, the other',
  '// guards an arbitrary closure. They MUST NOT share a symbol in Rust.',
  '',
  '#[derive(Debug, Clone)]',
  'pub struct CmpsblIsolateError { pub code: String, pub message: String, pub name: String }',
  '',
  '#[derive(Debug)]',
  'pub struct CmpsblIsolateResult<T> {',
  '    pub ok: bool,',
  '    pub value: Option<T>,',
  '    pub error: Option<CmpsblIsolateError>,',
  '    pub duration_ms: u64,',
  '}',
  '',
  ...
  '}',
  '',
  'pub fn cmpsbl_isolate<T, F>(name: &str, f: F) -> CmpsblIsolateResult<T>',
  ...
  '        return match f() {',
  '            Ok(v)  => CmpsblIsolateResult { ok: true,  value: Some(v), error: None, duration_ms: dur() },',
  '            Err(e) => CmpsblIsolateResult { ok: false, value: None, error: Some(CmpsblIsolateError { code: "EXEC_ERROR".into(), message: e, name: name.into() }), duration_ms: dur() },',
  '        };',
  '    }',
  '',
...
// ── Ruby Tier 1 Kernel Hardening Bodies (Components #15–#20) ────────────────

registerNative('circuit-breaker-recoverable', 'ruby', () => [
  '# CMPSBL® Ascension Kernel — Recoverable Circuit Breaker',
  '# Three-state FSM with cooldown + half-open probing. Thread-safe via Mutex.',
  'module CmpsblBreakers',
  '  STATES = [:closed, :open, :half_open].freeze',
  '  @mutex = Mutex.new',
  '  @breakers = {}',
  '  DEFAULTS = { failure_threshold: 5, cooldown_ms: 30_000, success_threshold: 1 }.freeze',
  '',
  '  def self.now_ms; (Time.now.to_f * 1000).to_i; end',
  '',
  '  def self.get(name)',
  '    @mutex.synchronize do',
  '      @breakers[name] ||= { state: :closed, failures: 0, successes: 0, opened_at: 0, config: DEFAULTS.dup }',
  '    end',
  '  end',
  '',
  '  def self.allow?(name)',
  ...
  '      when :open',
  '        if now_ms - b[:opened_at] >= b[:config][:cooldown_ms]',
  '          b[:state] = :half_open; b[:successes] = 0; true',
  '        else false',
  '        end',
  ...
  '        b[:successes] += 1',
  '        if b[:successes] >= b[:config][:success_threshold]',
  ...
  '    @mutex.synchronize do',
  '      b[:failures] += 1',
  '      if b[:state] == :half_open || b[:failures] >= b[:config][:failure_threshold]',
  '        b[:state] = :open; b[:opened_at] = now_ms',
  '      end',
  '    end',
  '  end',
  '',
  '  def self.state(name); get(name); @mutex.synchronize { @breakers[name][:state] }; end',
  '  def self.reset(name); @mutex.synchronize { @breakers.delete(name) }; end',
  '  def self.reset_all; @mutex.synchronize { @breakers.clear }; end',
  'end',
].join('\n'));

registerNative('causality-tracker', 'ruby', () => [
  '# CMPSBL® Ascension Kernel — Causality Tracker',
  '# Parent/child span graph (FNV-1a IDs). Bounded ring (2048). Thread-safe.',
  'module CmpsblCausality',
  '  @mutex = Mutex.new',
  '  @spans = []',
  '  @current = nil',
  '  RING_MAX = 2048',
  ...
  '  end',
  '',
  '  def self.start(name, attrs = {})',
  '    @mutex.synchronize do',
  '      parent = @current',
  '      seq = @spans.length + 1',
  '      id = fnv1a("#{name}|#{seq}|#{(Time.now.to_f * 1e6).to_i}")',
  '      span = { id: id, parent_id: parent, name: name, attrs: attrs, start_ms: (Time.now.to_f * 1000).to_i, end_ms: nil }',
  '      @spans << span',
  '      @spans.shift if @spans.length > RING_MAX',
  '      @current = id',
  '      id',
  '    end',
  '  end',
  ...
  '  def self.finish(id)',
  '    @mutex.synchronize do',
  '      span = @spans.find { |s| s[:id] == id }',
  '      next unless span',
  '      span[:end_ms] = (Time.now.to_f * 1000).to_i',
  '      @current = span[:parent_id]',
  '    end',
  '  end',
  '',
  '  def self.spans; @mutex.synchronize { @spans.dup }; end',
  '  def self.reset; @mutex.synchronize { @spans.clear; @current = nil }; end',
  'end',
].join('\n'));

registerNative('backpressure-governor', 'ruby', () => [
  '# CMPSBL® Ascension Kernel — Backpressure Governor',
  '# Bounded queue with admission control + load shedding. Thread-safe.',
  'module CmpsblBackpressure',
  '  @mutex = Mutex.new',
  '  @max_inflight = 64',
  '  @inflight = 0',
  '  @queued = 0',
  ...
  '  @admitted = 0',
  '',
  '  def self.configure(max_inflight: 64, max_queue: 256)',
  '    @mutex.synchronize { @max_inflight = max_inflight; @max_queue = max_queue }',
  '  end',
  '',
  '  def self.try_admit',
  '    @mutex.synchronize do',
  '      if @inflight < @max_inflight',
  '        @inflight += 1; @admitted += 1; true',
  '      elsif @queued < @max_queue',
  '        @queued += 1; @admitted += 1; true',
  '      else',
  '        @rejected += 1; false',
  '      end',
  '    end',
  ...
  '  def self.release',
  '    @mutex.synchronize do',
  '      if @queued > 0 then @queued -= 1 else @inflight = [@inflight - 1, 0].max end',
  '    end',
  '  end',
  '',
  '  def self.snapshot',
  '    @mutex.synchronize do',
  '      { inflight: @inflight, queued: @queued, admitted: @admitted, rejected: @rejected,',
  '        max_inflight: @max_inflight, max_queue: @max_queue,',
  '        utilization: @max_inflight.zero? ? 0.0 : (@inflight.to_f / @max_inflight) }',
  '    end',
  '  end',
  '',
  '  def self.reset; @mutex.synchronize { @inflight = 0; @queued = 0; @admitted = 0; @rejected = 0 }; end',
  'end',
].join('\n'));

registerNative('determinism-fingerprint', 'ruby', () => [
  '# CMPSBL® Ascension Kernel — Determinism Fingerprint',
  '# FNV-1a fingerprint over (name, args, result) for replay verification.',
  'module CmpsblFingerprint',
  '  def self.fnv1a(s)',
  '    h = 0x811c9dc5',
  '    s.each_byte { |b| h = ((h ^ b) * 0x01000193) & 0xffffffff }',
  '    format("%08x", h)',
  '  end',
  '',
  '  def self.compute(name, args, result)',
  '    fnv1a("#{name}|#{args.inspect}|#{result.inspect}")',
  '  end',
  '',
  '  def self.matches?(expected, name, args, result)',
  '    compute(name, args, result) == expected',
  '  end',
  'end',
].join('\n'));

registerNative('contract-versioning', 'ruby', () => [
  '# CMPSBL® Ascension Kernel — Contract Versioning',
  '# Semver-tagged contracts; compatibility lookups; deprecation tracking.',
  'module CmpsblContractVersions',
  '  @mutex = Mutex.new',
  '  @registry = {}',
  '',
  '  def self.register(name, version, schema, deprecated: false)',
  '    @mutex.synchronize do',
  '      @registry[name] ||= {}',
  '      @registry[name][version] = { schema: schema, deprecated: deprecated, registered_at: (Time.now.to_f * 1000).to_i }',
  '    end',
  '  end',
  '',
  '  def self.get(name, version)',
  '    @mutex.synchronize { (@registry[name] || {})[version] }',
  '  end',
  '',
  '  def self.versions(name)',
  '    @mutex.synchronize { (@registry[name] || {}).keys.sort }',
  '  end',
  '',
  ...
  '  end',
  '',
  '  def self.deprecated?(name, version)',
  '    v = get(name, version); v && v[:deprecated]',
  '  end',
  '',
  '  def self.reset; @mutex.synchronize { @registry.clear }; end',
  'end',
].join('\n'));

registerNative('saturation-metrics', 'ruby', () => [
  '# CMPSBL® Ascension Kernel — Saturation Metrics',
  '# Per-resource utilization (CPU, memory, queue) with rolling EWMA.',
  'module CmpsblSaturation',
  '  @mutex = Mutex.new',
  '  @gauges = {}',
  '  ALPHA = 0.3',
  '',
  '  def self.observe(resource, value)',
  '    @mutex.synchronize do',
  '      g = @gauges[resource] ||= { current: 0.0, ewma: 0.0, peak: 0.0, samples: 0 }',
  '      g[:current] = value.to_f',
  '      g[:ewma] = g[:samples].zero? ? value.to_f : (ALPHA * value.to_f + (1 - ALPHA) * g[:ewma])',
  '      g[:peak] = [g[:peak], value.to_f].max',
  '      g[:samples] += 1',
  '    end',
  '  end',
  '',
  '  def self.snapshot(resource = nil)',
  '    @mutex.synchronize do',
  '      resource ? (@gauges[resource] && @gauges[resource].dup) : @gauges.transform_values(&:dup)',
  '    end',
  '  end',
  '',
  '  def self.saturated?(resource, threshold = 0.85)',
  '    g = @gauges[resource]; g && g[:ewma] >= threshold',
  '  end',
  '',
  '  def self.reset; @mutex.synchronize { @gauges.clear }; end',
  'end',
].join('\n'));
