...
function generateIntegrationGuide(product: ProductZipInput): string {
  const kindLabel = product.kind === 'engine' ? 'Engine' : 'Agent';
  return `# Integration Guide — ${product.name}

## Three Ways to Use This ${kindLabel}

### 1. Drop-In (Standalone ZIP)

Copy this folder into your project and import directly.
The embedded Mini-Runtime™ handles everything — zero external dependencies.

\`\`\`typescript
import { init, createPipeline } from './src/${product.slug}';

const instance = init({ offline: true }); // fully standalone
const pipeline = createPipeline(instance);
const result = await pipeline.execute({ your: 'data' });
console.log(result.output);
\`\`\`

### 2. NPM / SDK (Recommended for Production)
...
const client = new CMPSBL();
// ${kindLabel} activates automatically based on your subscription
// Full substrate primitives available when online
\`\`\`

**Related packages:**
- [@cmpsbl/sdk](https://www.npmjs.com/package/@cmpsbl/sdk) — Full SDK
- [@cmpsbl/runtime](https://www.npmjs.com/package/@cmpsbl/runtime) — Mini-Runtime™
- [@cmpsbl/cli](https://www.npmjs.com/package/@cmpsbl/cli) — Terminal tools
- [@cmpsbl/types](https://www.npmjs.com/package/@cmpsbl/types) — TypeScript types
...
cmpsbl login
cmpsbl ${product.kind === 'engine' ? 'engines' : 'agents'} activate ${product.slug}
cmpsbl status  # Verify activation
\`\`\`
...
The Mini-Runtime™ automatically connects to the CMPSBL® Substrate when available:

| Mode | Primitives | Latency | Use Case |
|------|-----------|---------|----------|
| **Network** | Full 40-primitive deep effects | ~50ms | Production with substrate |
| **Hybrid** | Auto-fallback on failure | Variable | Unreliable networks |
| **Offline** | Local deterministic fallback | <1ms | Air-gapped / embedded |

\`\`\`typescript
import { configureEndpoint, getRuntimeMode } from './_runtime/standalone-runtime';

// Check current mode
console.log(getRuntimeMode()); // 'hybrid' (default)

// Force offline
configureEndpoint(null);

// Custom endpoint (self-hosted substrate)
configureEndpoint('https://your-substrate.example.com/v1/substrate/primitive');
\`\`\`
...
The following are sealed and protected:
