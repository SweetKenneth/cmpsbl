...
const DISCOVERY_TEMPLATES: DiscoveryTemplate[] = [
  ...
  { namePattern: 'Self-Correcting Task Orchestrator', descriptionPattern: 'Autonomous task pipeline with built-in self-correction checkpoints, rollback capabilities, and progressive verification gates.', category: 'autonomy', primaryPrimitives: ['MANDATE', 'OVERSEER', 'DELEGATE'], minChainLength: 6, maxChainLength: 8, cjpiBias: 32 },
  ...
  { namePattern: 'Evidence-Based Decision Support System', descriptionPattern: 'Research-driven decision support that gathers evidence, evaluates credibility, and presents structured analyses with confidence intervals.', category: 'research', primaryPrimitives: ['RECONN', 'REASON', 'ENVOY'], minChainLength: 4, maxChainLength: 6, cjpiBias: 17 },

  // ═══ COMMUNICATION (mid CJPI) ═══
  { namePattern: 'Audience-Adaptive Report Generator', descriptionPattern: 'Generates reports with automatically adjusted complexity, terminology, and detail level based on recipient expertise analysis.', category: 'communication', primaryPrimitives: ['SCRIBE', 'ENVOY', 'REASON'], minChainLength: 3, maxChainLength: 5, cjpiBias: 14 },
  { namePattern: 'Multi-Channel Notification Orchestrator', descriptionPattern: 'Intelligent notification routing across channels with priority-based batching, user preference learning, and alert fatigue prevention.', category: 'communication', primaryPrimitives: ['ENVOY', 'UPLINK', 'INCENTIVE'], minChainLength: 3, maxChainLength: 5, cjpiBias: 12 },
  ...
  { namePattern: 'Reward Signal Optimization Loop', descriptionPattern: 'Closed-loop reward function tuning that measures behavioral impact of incentive changes and auto-calibrates reinforcement parameters.', category: 'learning', primaryPrimitives: ['INCENTIVE', 'SCHOLAR', 'OVERSEER'], minChainLength: 3, maxChainLength: 5, cjpiBias: 11 },
  ...
  { namePattern: 'Tool Call Error Recovery System', descriptionPattern: 'Robust error handling for failed tool calls with parameter mutation, alternative tool selection, and partial result extraction.', category: 'tooling', primaryPrimitives: ['TOOLKIT', 'OVERSEER', 'OPERATOR'], minChainLength: 3, maxChainLength: 5, cjpiBias: 10 },

  // ═══ RESILIENCE (lower CJPI, junkyard candidates) ═══
  { namePattern: 'Fleet Circuit Breaker Network', descriptionPattern: 'Distributed circuit breaker implementation preventing cascade failures across the agent fleet with configurable trip thresholds.', category: 'resilience', primaryPrimitives: ['OVERSEER', 'DEFENSE'], minChainLength: 2, maxChainLength: 4, cjpiBias: 8 },
  { namePattern: 'Agent State Reconstruction Tool', descriptionPattern: 'Rebuilds agent state from persistent checkpoints after unexpected failures with minimal context loss.', category: 'resilience', primaryPrimitives: ['OVERSEER', 'ANCHOR'], minChainLength: 2, maxChainLength: 3, cjpiBias: 4 },
  { namePattern: 'Degraded Mode Capability Shedder', descriptionPattern: 'Priority-based feature shedding during system stress ensuring core mission capabilities remain available.', category: 'resilience', primaryPrimitives: ['OVERSEER', 'MANDATE'], minChainLength: 2, maxChainLength: 3, cjpiBias: 2 },
  { namePattern: 'Health Telemetry Aggregator', descriptionPattern: 'Lightweight telemetry collection for agent health metrics with EMA smoothing and anomaly flagging.', category: 'resilience', primaryPrimitives: ['OVERSEER', 'BEACON'], minChainLength: 2, maxChainLength: 3, cjpiBias: 0 },
  { namePattern: 'Task Timeout Watchdog', descriptionPattern: 'Configurable timeout enforcement for long-running agent tasks with graceful cancellation and partial result salvage.', category: 'resilience', primaryPrimitives: ['OPERATOR', 'OVERSEER'], minChainLength: 2, maxChainLength: 3, cjpiBias: -2 },
