...
function renderRustAttachmentBlock(
  ...
  const lines: string[] = [
    ...
    '    }',
    '',
    '    // ─── Tower Layer opt-in (Axum middleware) ───',
    '    // Wrap your Router with `.layer(cmpsbl_attach::CmpsblTowerLayer::new("capability"))`',
    '    // to flow every request through `cmpsbl_execute` pre/post.',
    '    // Requires the `tower` crate (already a transitive dep of axum).',
    '    #[derive(Clone)]',
    '    pub struct CmpsblTowerLayer {',
    '        capability: std::sync::Arc<str>,',
    '    }',
    '    impl CmpsblTowerLayer {',
    '        pub fn new(capability: &str) -> Self {',
    '            Self { capability: std::sync::Arc::from(capability) }',
    '        }',
    '    }',
    '    impl<S> tower::Layer<S> for CmpsblTowerLayer {',
    '        type Service = CmpsblTowerService<S>;',
    '        fn layer(&self, inner: S) -> Self::Service {',
    '            CmpsblTowerService { inner, capability: self.capability.clone() }',
    '        }',
    '    }',
    '    #[derive(Clone)]',
    '    pub struct CmpsblTowerService<S> {',
    '        inner: S,',
    '        capability: std::sync::Arc<str>,',
    '    }',
    "    impl<S, Req> tower::Service<Req> for CmpsblTowerService<S>",
    '    where',
    ...
    '            let cap = self.capability.clone();',
    '            let mut inner = self.inner.clone();',
    '            Box::pin(async move {',
    '                {',
    '                    let mut input = std::collections::HashMap::new();',
    '                    input.insert("_cmpsbl_phase".to_string(), JsonValue::String("pre".into()));',
    '                    let _ = cmpsbl_execute(&cap, input);',
    '                }',
    ...
    '// For Axum: `.route("/x", get(cmpsbl_<name>))` or `.layer(cmpsbl_attach::CmpsblTowerLayer::new("cap"))`.',
  ];
  for (const w of wrappable) {
    if (w.isAsync) {
      lines.push(
