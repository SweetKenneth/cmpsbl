...
4. **Batch requests** — use CORTEX Engine memory chains to combine multiple operations
...
Rate limiting is enforced by the Ironclad hardening fabric — a multi-layer protection system with:

- Per-key limits
- Per-IP limits
- Per-primitive bulkheads
- Behavioral analysis for automated attack detection
- 30-second auto-restore loop for resilience
