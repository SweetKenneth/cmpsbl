...
Return JSON with all fields.`;
    ...
    const result = await callFreeTierAI(hypothesisPrompt, {
      systemPrompt: 'You are a hypothesis testing expert. Evaluate claims with rigorous IF-THEN logic.',
      temperature: 0.5
    });
    ...
    await supabase.from('brain_events').insert({
      ...
      data: { claim, strategy, hypothesis_test, provider: result.provider },
    ...
    return new Response(
      JSON.stringify({ success: true, hypothesis_test, provider: result.provider }),
