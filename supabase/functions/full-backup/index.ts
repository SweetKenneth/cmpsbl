...
const SKIP_TABLES = new Set<string>([
  ...
  'brain_events',              // ~88K rows, regenerated by BRAIN Primitive
  'ai_usage_log',              // ~157K rows, telemetry logs
  'brain_maintenance_log',     // ~21K rows, maintenance telemetry
  'analytics_events',          // ~15K rows, event telemetry
  'brain_memory_pruned',       // ~5K rows, pruning artifacts
  'brain_memory_cold',         // ~9.9K rows, cold-tier (regenerated via tiering)
  'site_page_views',           // ~8K rows, analytics regenerated
  'brain_memory_warm',         // ~7.6K rows, warm-tier (regenerated via tiering)
  'learning_queries',          // transient learning data
  'defense_events',            // ~2.2K rows, regenerated by DEFENSE Layer
  'brain_reflection_log',      // ~1K rows, transient reflections
  'brain_metrics',             // ~1.1K rows, regenerated metrics
  'pf_brain_anomalies',        // ~2K rows, anomaly telemetry
  'site_sessions',             // ~1.6K rows, analytics sessions
  'brain_graph_edges',         // ~2.7K rows, regenerated graph
  'mesh_comms',                // ~2.1K rows, transient mesh comms
  'brain_distillation_runs',   // ~523 rows, transient runs
  'brain_reasoning_traces',    // ~424 rows, transient traces
  'decode_search_results',     // ~752 rows, transient search cache
  'agency_task_logs',          // ~917 rows, task telemetry
  'ai_daily_quota',            // daily counters reset
  'nexus_hourly_snapshots',    // regenerated snapshots
  'nexus_traces',              // transient traces
  'client_error_log',          // error telemetry
