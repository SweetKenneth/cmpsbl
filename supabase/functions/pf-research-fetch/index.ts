...
serve(async (req) => {
  ...
  try {
    ...
    if (groqKey && groqWithinBudget) {
      try {
        console.log(`Using Groq (Llama 3.3 70B) - $${groqCostToday.toFixed(4)}/$1.00 spent today`);
        ...
        console.log(`✅ Groq [$${(groqCostToday + costIncrement).toFixed(4)}/$1.00] | Topic: ${query.metadata?.topic || 'general'} | Relevance: ${relevance.toFixed(2)} | Insights: ${insights.length}`);
    ...
    } else if (!groqWithinBudget) {
      console.error(`Groq budget exhausted: $${groqCostToday.toFixed(2)}/$1.00`);
    }
...
// fetchLovableAI removed - now using Groq as primary via fetchGroqFallback
