  ...
  SELECT jsonb_build_object(
    'core', (
      SELECT jsonb_build_object(
        ...
        -- FIXED: Use is_bounce column (set by heartbeat tracker) instead of page view count
        ...
        -- FIXED: returning_visitors = fingerprints with 2+ sessions
        'returning_visitors', (
          SELECT COUNT(*) FROM (
        ...
        -- FIXED: new_visitors = fingerprints with exactly 1 session
        'new_visitors', (
          SELECT COUNT(*) FROM (
            SELECT fingerprint_hash FROM site_sessions
            WHERE started_at >= p_start_date AND started_at <= p_end_date
