...
DECLARE
  v_excluded_fps text[];
  v_result jsonb;
  v_max_session_ms numeric := 1800000; -- 30 minutes cap for avg calculation
BEGIN
  ...
  SELECT jsonb_build_object(
    'core', (
      SELECT jsonb_build_object(
        ...
        'avg_session_duration_ms', ROUND(COALESCE(AVG(LEAST(NULLIF(s.total_duration_ms, 0), v_max_session_ms))::numeric, 0)),
    ...
    'top_pages', (
      ...
      FROM (
        SELECT 
          ...
          COUNT(*) AS views,
          ROUND(COALESCE(AVG(LEAST(NULLIF(pv.time_on_page_ms, 0), v_max_session_ms))::numeric, 0)) AS avg_time,
