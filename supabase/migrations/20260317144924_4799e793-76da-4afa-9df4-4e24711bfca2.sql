...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'top_referrers', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object('domain', ref_sub.domain, 'sessions', ref_sub.sessions) ORDER BY ref_sub.sessions DESC), '[]'::jsonb)
      FROM (
        SELECT 
          COALESCE(NULLIF(s.referrer_domain, ''), 'Direct') AS domain,
          COUNT(*) AS sessions
        FROM site_sessions s
        WHERE s.started_at >= p_start_date AND s.started_at <= p_end_date
          AND s.fingerprint_hash IS NOT NULL
          AND NOT (s.fingerprint_hash = ANY(v_excluded_fps))
        GROUP BY COALESCE(NULLIF(s.referrer_domain, ''), 'Direct')
        ORDER BY COUNT(*) DESC
        LIMIT 15
      ) ref_sub
    ),
    'devices', (
      ...
      FROM (
        SELECT s.device_type, COUNT(*) AS cnt
        FROM site_sessions s
        WHERE s.started_at >= p_start_date AND s.started_at <= p_end_date
          AND s.fingerprint_hash IS NOT NULL
          AND NOT (s.fingerprint_hash = ANY(v_excluded_fps))
    ...
    'countries', (
      SELECT COALESCE(jsonb_agg(jsonb_build_object('code', c_sub.country_code, 'count', c_sub.cnt) ORDER BY c_sub.cnt DESC), '[]'::jsonb)
      FROM (
        SELECT COALESCE(s.country_code, 'Unknown') AS country_code, COUNT(*) AS cnt
        FROM site_sessions s
        WHERE s.started_at >= p_start_date AND s.started_at <= p_end_date
          AND s.fingerprint_hash IS NOT NULL
          AND NOT (s.fingerprint_hash = ANY(v_excluded_fps))
        GROUP BY COALESCE(s.country_code, 'Unknown')
        ORDER BY COUNT(*) DESC
        LIMIT 20
      ) c_sub
    )
