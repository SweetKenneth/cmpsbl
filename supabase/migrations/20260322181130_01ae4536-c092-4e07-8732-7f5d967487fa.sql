...
DECLARE
  ...
  v_count bigint;
BEGIN
  -- brain_events: keep 14 days (high-volume telemetry, deep-maintenance handles 7d)
  ...
  -- brain_memory_pruned: keep 7 days (already demoted, just cleanup)
  ...
  -- ai_usage_log: keep 30 days (was 90 — tightened for bloat control)
  ...
  -- nexus_traces: keep 14 days (was 30 — tightened)
  ...
  -- nexus_cost_ledger: keep 180 days (financial records)
  ...
  -- audit_logs: keep 365 days (compliance)
  ...
  -- agency_agent_telemetry: keep 60 days (was 90)
  ...
  -- defense_events: keep 30 days
  DELETE FROM public.defense_events WHERE created_at < now() - INTERVAL '30 days';
  GET DIAGNOSTICS v_count = ROW_COUNT;
  v_result := v_result || jsonb_build_object('defense_events_deleted', v_count);

  -- pf_brain_anomalies: keep 30 days
  DELETE FROM public.pf_brain_anomalies WHERE detected_at < now() - INTERVAL '30 days';
  GET DIAGNOSTICS v_count = ROW_COUNT;
