-- Allow memory_cold artifact type (was being silently rejected)
...
ALTER TABLE public.brain_embeddings ADD CONSTRAINT brain_embeddings_artifact_type_check 
  CHECK (artifact_type = ANY (ARRAY['crystal'::text, 'trace'::text, 'heuristic'::text, 'memory_hot'::text, 'memory_warm'::text, 'memory_cold'::text]));